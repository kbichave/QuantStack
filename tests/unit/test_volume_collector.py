...
class TestVolumeCollectorCore:
    def test_returns_dict(self, run_async):
        store = _make_store()
        result = run_async(collect_volume("SPY", store))
    ...
    def test_returns_empty_on_insufficient_bars(self, run_async):
        store = MagicMock()
        store.load_ohlcv.return_value = None
        result = run_async(collect_volume("SPY", store))
        assert result == {}

    def test_volume_trend_valid_values(self, run_async):
        store = _make_store()
        result = run_async(collect_volume("SPY", store))
...
class TestVolumeCollectorVPOC:
    def test_vpoc_key_present(self, run_async):
        store = _make_store()
        result = run_async(collect_volume("SPY", store))
        assert "vpoc" in result

    def test_vpoc_within_price_range(self, run_async):
        store = _make_store()
        result = run_async(collect_volume("SPY", store))
    ...
    def test_vah_above_val(self, run_async):
        store = _make_store()
        result = run_async(collect_volume("SPY", store))
        vah = result.get("vah")
        val = result.get("val")
    ...
    def test_price_in_value_area_is_binary(self, run_async):
        store = _make_store()
        result = run_async(collect_volume("SPY", store))
...
class TestVolumeCollectorAnchoredVWAP:
    def test_avwap_key_present(self, run_async):
        store = _make_store()
        result = run_async(collect_volume("SPY", store))
        assert "avwap" in result

    def test_avwap_positive(self, run_async):
        store = _make_store()
        result = run_async(collect_volume("SPY", store))
    ...
    def test_above_avwap_binary(self, run_async):
        store = _make_store()
        result = run_async(collect_volume("SPY", store))
...
class TestVolumeCollectorMicrostructure:
    def test_amihud_key_present(self, run_async):
        store = _make_store()
        result = run_async(collect_volume("SPY", store))
        assert "amihud" in result

    def test_amihud_non_negative(self, run_async):
        store = _make_store()
        result = run_async(collect_volume("SPY", store))
    ...
    def test_roll_spread_pct_present(self, run_async):
        store = _make_store()
        result = run_async(collect_volume("SPY", store))
        # May be NaN for trending data (positive covariance) — key should still exist
        assert "roll_spread_pct" in result

    def test_cs_spread_pct_present(self, run_async):
        store = _make_store()
        result = run_async(collect_volume("SPY", store))
        assert "cs_spread_pct" in result

    def test_rv_overnight_ratio_in_range(self, run_async):
        store = _make_store()
        result = run_async(collect_volume("SPY", store))
    ...
    def test_gap_keys_present(self, run_async):
        store = _make_store()
        result = run_async(collect_volume("SPY", store))
        for key in ("gap_up", "gap_down", "gap_persisted"):
            assert key in result

    def test_gap_up_down_mutually_exclusive(self, run_async):
        store = _make_store()
        result = run_async(collect_volume("SPY", store))
        if "gap_up" in result and "gap_down" in result:
            assert not (result["gap_up"] == 1 and result["gap_down"] == 1)

    def test_no_exceptions_with_constant_prices(self, run_async):
...
class TestVolumeCollectorVWAPDeviation:
    def test_vwap_deviation_key_present(self, run_async):
        store = _make_store()
        result = run_async(collect_volume("SPY", store))
        assert "vwap_deviation" in result

    def test_vwap_deviation_zscore_key_present(self, run_async):
        store = _make_store()
        result = run_async(collect_volume("SPY", store))
        assert "vwap_deviation_zscore" in result

    def test_vwap_deviation_reasonable_range(self, run_async):
