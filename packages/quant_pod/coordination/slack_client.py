...
class SlackClient:
    ...
    def post_agent_report(
        ...
        blocks = [
            {"type": "header", "text": {"type": "plain_text", "text": f"{agent_name.replace('_', ' ').title()} {'— ' + symbol if symbol else ''}"}},
            {"type": "section", "text": {"type": "mrkdwn", "text": f"*Summary:* {summary}"}},
            {"type": "divider"},
            {"type": "section", "text": {"type": "mrkdwn", "text": self._truncate(full_report, 2900)}},
            {"type": "context", "elements": [{"type": "mrkdwn", "text": f":clock1: {datetime.now(timezone.utc).strftime('%H:%M UTC')}"}]},
        ]
    ...
    def post_trade(self, trade_result: dict[str, Any]) -> str | None:
        ...
        if is_options:
            ...
            premium = trade_result.get("fill_premium_per_contract", trade_result.get("total_premium", 0))
            contracts = trade_result.get("contracts", trade_result.get("contracts_filled", 1))
    ...
    def post_portfolio_summary(self, snapshot: dict[str, Any]) -> str | None:
        ...
        pnl_emoji = ":chart_with_upwards_trend:" if daily_pnl >= 0 else ":chart_with_downwards_trend:"
        text = (
    ...
    def post_signal_brief(
        ...
        bias_emoji = {"bullish": ":green_circle:", "bearish": ":red_circle:", "neutral": ":white_circle:"}.get(bias, ":grey_question:")
    ...
    def post_strategy_event(
        ...
        emoji_map = {"promoted": ":arrow_up:", "retired": ":coffin:", "demoted": ":arrow_down:", "registered": ":new:"}
