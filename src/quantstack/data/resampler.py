...
from quantstack.config.timeframes import TIMEFRAME_HIERARCHY, Timeframe
...
class TimeframeResampler:
    ...
    def resample_to_higher_tf(
        ...
        logger.debug(
            f"Resampled {len(df)} bars to {len(resampled)} {target_tf.value} bars"
        )
        return resampled
    ...
    def build_multi_timeframe_dataset(
        ...
        if include_weekly:
            if Timeframe.D1 in result:
                result[Timeframe.W1] = self.build_weekly_from_daily(
                    result[Timeframe.D1]
                )
            else:
