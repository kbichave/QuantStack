...
class OptionsFlowClient:
    ...
    def _compute_signal(
        ...
        total_score = min(
            100.0, pc_score + unusual_score_component + premium_score + iv_score
        )
...
def _make_options_flow_tool():
    ...
    class OptionsFlowInput(PydanticBaseModel):
        symbol: str = PydanticField(
            ..., description="Ticker symbol to analyse, e.g. 'SPY'"
        )
    ...
    class OptionsFlowTool(BaseTool):
        ...
        def _run(self, symbol: str, expiry_within_days: int = 45) -> str:
            ...
            signal = client.get_flow(
                symbol.upper(), expiry_within_days=expiry_within_days
            )
