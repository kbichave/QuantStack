...
class WalkForwardRequest(BaseModel):
    ...
    test_size: int = Field(default=252, description="Bars per test fold (~1 year daily)")
    min_train_size: int = Field(default=504, description="Minimum training bars (~2 years)")
    gap: int = Field(default=0, description="Embargo bars between train and test")
    expanding: bool = True
    initial_capital: float = 100_000.0
    position_size_pct: float = 0.10
