...
class PortfolioOptimizerAgent:
    ...
    def optimize(
        ...
        if missing:
            logger.debug(f"[PortOpt] Symbols missing from returns_df — fallback: {missing}")
            # Use signal-proportional for missing symbols; optimize only available
            for sym in missing:
                result_weights[sym] = self._single_signal_weight(signals[sym], len(buy_symbols))
        ...
        if len(aligned_returns) < _MIN_BARS_FOR_COV:
            result_weights.update(
                self._signal_proportional_weights({s: signals[s] for s in available_cols})
            )
            return result_weights
        ...
        try:
            ...
            logger.warning(f"[PortOpt] Covariance build failed ({cov_err}) — using fallback")
            result_weights.update(self._signal_proportional_weights(signals))
            return result_weights
