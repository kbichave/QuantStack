...
class TestSupertrend:
    ...
    def test_no_lookahead(self):
        ...
        # Add one new bar
        extra = pd.concat([hi, pd.Series([hi.iloc[-1] * 1.01], index=[hi.index[-1] + pd.Timedelta("1D")])])
        extra_lo = pd.concat([lo, pd.Series([lo.iloc[-1]], index=[lo.index[-1] + pd.Timedelta("1D")])])
        extra_cl = pd.concat([cl, pd.Series([cl.iloc[-1] * 1.01], index=[cl.index[-1] + pd.Timedelta("1D")])])
...
class TestIchimokuCloud:
    ...
    def test_cloud_signal_columns(self):
        hi, lo, cl = _ohlcv(n=120)
        result = IchimokuCloud().compute(hi, lo, cl)
        for col in ("cloud_bullish", "price_above_cloud", "price_below_cloud", "tenkan_above_kijun"):
            assert col in result.columns
