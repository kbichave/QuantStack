...
def _ohlcv(
    ...
    high = pd.Series(
        np.maximum(open_, close) + np.abs(np.random.randn(n) * 0.5), index=dates
    )
    low = pd.Series(
        np.minimum(open_, close) - np.abs(np.random.randn(n) * 0.5), index=dates
    )
...
def _mean_reverting_series(
    n: int = 500, theta: float = 0.1, mu: float = 100.0
) -> pd.Series:
...
class TestHurstExponent:
    ...
    def test_trending_series_high_hurst(self):
        ...
        if len(valid) > 0:
            assert (
                valid.mean() > 0.45
            ), f"Expected H > 0.45 for trending, got {valid.mean()}"
...
class TestOUHalfLife:
    ...
    def test_trending_not_mean_reverting(self):
        ...
        assert (
            result_mr["ou_half_life_valid"].sum() >= result["ou_half_life_valid"].sum()
        )
...
class TestEntropyFeatures:
    ...
    def test_constant_series_low_entropy(self):
        ...
        if len(valid) > 0:
            ...
            assert (
                valid.iloc[-1] < 2.5
            ), f"Expected low entropy for constant returns, got {valid.iloc[-1]}"
