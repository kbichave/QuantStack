...
def probability_of_backtest_overfitting(
    ...
    if T < 20 or N < 2:
        logger.warning(f"PBO requires at least 20 time steps and 2 strategies; got {T}×{N}")
        return PBOResult(
            pbo=0.5,
            is_overfit=False,
            n_paths=0,
    ...
    for test_group_ids in combinations(range(n_splits), n_test_splits):
        ...
        train_idx = sorted(i for i in range(T) if i not in all_test and i not in purge_idx)
