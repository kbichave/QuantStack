...
class CausalFilter:
    ...
    def __init__(
        ...
        if correction not in ("bonferroni", "holm"):
            raise ValueError(f"correction must be 'bonferroni' or 'holm', got '{correction}'")
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series) -> "CausalFilter":
        ...
        logger.info(f"CausalFilter: testing {n_features} features (max_lag={self.max_lag})")
    ...
    def _test_feature(
        ...
        if not is_stationary and self.auto_difference:
            ...
            adf_result2 = adf_test(feat_for_test, significance_level=self.stationarity_alpha)
    ...
    def _compute_transfer_entropy_batch(
        ...
        def _compute_single(feat_name: str) -> tuple[str, float, float]:
            ...
            te_val, te_p = _te_permutation_test(
                feat_arr[mask], y_arr[mask], lag=1,
                n_permutations=self.te_n_permutations, rng=rng,
            )
        ...
        if self.n_jobs > 1:
            with ProcessPoolExecutor(max_workers=self.n_jobs) as pool:
                futures = {
                    pool.submit(_compute_single, r.feature_name): r
                    for r in results
                }
