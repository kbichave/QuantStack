...
class TestCrossAssetCollectorCore:
    ...
    def test_empty_when_spy_missing(self):
        store = MagicMock()
        store.load_ohlcv.return_value = None
        result = _run(collect_cross_asset("AAPL", store))
        assert result == {}

    def test_empty_when_spy_insufficient_bars(self):
        store = MagicMock()
...
class TestCrossAssetETFSignals:
    ...
    def test_iwm_vs_spy_key_present(self):
        result = _run(collect_cross_asset("AAPL", _make_store()))
        assert "iwm_vs_spy_5d" in result

    def test_tlt_return_key_present(self):
        result = _run(collect_cross_asset("AAPL", _make_store()))
        assert "tlt_return_5d" in result

    def test_gld_return_key_present(self):
        result = _run(collect_cross_asset("AAPL", _make_store()))
        assert "gld_return_5d" in result

    def test_spy_regime_is_float_or_none(self):
        result = _run(collect_cross_asset("AAPL", _make_store()))
...
class TestCrossAssetRiskScore:
    ...
    def test_risk_on_score_is_float(self):
        result = _run(collect_cross_asset("AAPL", _make_store()))
...
class TestCrossAssetFuturesBasis:
    ...
    def test_es_basis_zscore_key_present(self):
        result = _run(collect_cross_asset("AAPL", _make_store()))
        assert "es_basis_zscore" in result

    def test_es_contango_binary(self):
        result = _run(collect_cross_asset("AAPL", _make_store()))
        if "es_contango" in result:
            assert result["es_contango"] in (0, 1)

    def test_es_basis_pct_is_float_or_none(self):
        result = _run(collect_cross_asset("AAPL", _make_store()))
        v = result.get("es_basis_pct")
        assert v is None or isinstance(v, float)

    def test_es_basis_absent_when_es_missing(self):
...
class TestCrossAssetVVIX:
    ...
    def test_vvix_above_sma20_binary(self):
        result = _run(collect_cross_asset("AAPL", _make_store()))
        if "vvix_above_sma20" in result:
            assert result["vvix_above_sma20"] in (0, 1)

    def test_vvix_positive(self):
        result = _run(collect_cross_asset("AAPL", _make_store()))
        v = result.get("vvix")
        if v is not None:
            assert v > 0

    def test_vvix_absent_when_missing(self):
    ...
    def test_vvix_absent_when_insufficient_bars(self):
