...
class AlpacaAdapter(AssetClassAdapter):
    ...
    def fetch_options_chain(
        ...
        try:
            ...
            logger.warning(
                f"[AlpacaAdapter] options snapshot failed for {symbol}: {exc}"
            )
            return None
        ...
        for contract_symbol, snap in snapshots.items():
            try:
                ...
                contracts.append(
                    {
                        ...
                        "volume": (
                            int(snap.day.volume)
                            if snap.day and snap.day.volume
                            else None
                        ),
                        "source": "alpaca",
                    }
                ...
                logger.debug(
                    f"[AlpacaAdapter] skipping contract {contract_symbol}: {exc}"
                )
                continue

        logger.info(
            f"[AlpacaAdapter] fetched {len(contracts)} option contracts for {symbol}"
        )
    ...
    def _fetch_native(
        ...
        request = StockBarsRequest(
            ...
            adjustment=Adjustment.ALL,  # Split + dividend adjusted for backtesting
        )
