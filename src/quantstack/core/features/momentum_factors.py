    ...
    def compute_single(
        ...
        if market_close is not None:
            ...
            if len(common) > 63:
                ...
                mkt_12_1 = (
                    market_close.reindex(common)
                    / market_close.reindex(common).shift(self.lookback)
                    - 1
                )
                mkt_1m = (
                    market_close.reindex(common)
                    / market_close.reindex(common).shift(self.skip)
                    - 1
                )
                mkt_mom = mkt_12_1 - mkt_1m
                residual = (mom_12_1.reindex(common) - beta * mkt_mom).reindex(
                    close.index
                )
...
class CrossSectionalDispersion:
    ...
    def compute(self, closes: dict[str, pd.Series]) -> pd.DataFrame:
        ...
        for i in range(self.window, len(ret_df)):
            window_data = ret_df.iloc[i - self.window + 1 : i + 1].dropna(
                axis=1, how="all"
            )
