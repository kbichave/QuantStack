...
class IntradayMonitorFlow:
    ...
    def _is_reversal(entry_trend: str, current_trend: str, side: str) -> bool:
        ...
        if entry_trend == "unknown" or current_trend == "unknown":
            return False
        if side == "long":
            return entry_trend == "trending_up" and current_trend in ("trending_down", "ranging")
        if side == "short":
            return entry_trend == "trending_down" and current_trend in ("trending_up", "ranging")
        return False
    ...
    def _run_degradation_check(self) -> str:
        ...
        try:
            ...
            severity_order = {"critical": 0, "warning": 1, "clean": 2, "insufficient_data": 3}
    ...
    def _compile_action_items(
        ...
        if alpha_status == "critical":
            ...
            items.append("ALPHA WARNING: IC decay detected. Review /skills/degradation.")
