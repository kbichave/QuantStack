...
from quantstack.core.execution.risk_gate import PreTradeRiskGate, RiskGateError
...
class AsyncExecutionLoop:
    ...
    async def _process_signal(self, features: IncrementalFeatures) -> None:
        try:
            ...
            logger.exception(
                f"[ExecLoop] SignalEvaluator raised for {features.symbol}: {exc}"
            )
        ...
        current_price = (
            self._price_fn(features.symbol) if self._price_fn else features.close
        )
        ...
        try:
            ...
            logger.exception(
                f"[ExecLoop] Unexpected error submitting {order.symbol}: {exc}"
            )
            self._stats.exceptions += 1
