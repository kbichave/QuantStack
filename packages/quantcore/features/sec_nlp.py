...
class EightKClassifier:
    ...
    def to_signal_dict(self, text: str) -> dict[str, Any]:
        ...
        return {
            "sec_8k_guidance":    1 if "guidance_raise" in cats else (-1 if "guidance_lower" in cats else 0),
...
def _tokenize(text: str) -> dict[str, int]:
    ...
    stops = {
        ...
        "this", "that", "with", "from", "our", "its", "may", "will", "can",
        "year", "quarter", "period", "fiscal", "million", "billion", "thousand",
    }
    freq: dict[str, int] = {}
    for w in words:
...
class MDADeltaAnalyzer:
    ...
    def analyze(self, text_prior: str, text_current: str) -> dict[str, Any]:
        ...
        sent_delta = round(sentiment_density(text_current) - sentiment_density(text_prior), 4)
