...
class TestEarningsTranscriptNLP:
    ...
    def test_positive_transcript_higher_tone(
        self, positive_transcript, negative_transcript
    ):
    ...
    def test_positive_count_exceeds_negative_for_bullish_text(
        self, positive_transcript
    ):
        result = EarningsTranscriptNLP().analyze(positive_transcript)
        assert result["positive_count"] > result["negative_count"]

    def test_negative_count_exceeds_positive_for_bearish_text(
        self, negative_transcript
    ):
    ...
    def test_batch_analyze_returns_dataframe(
        self, positive_transcript, negative_transcript
    ):
        records = [
