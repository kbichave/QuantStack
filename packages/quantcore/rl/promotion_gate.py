...
class PromotionResult:
    ...
    def summary(self) -> str:
        lines = [f"PromotionResult for {self.agent_type}: {'PASS' if self.passes else 'FAIL'}"]
...
class PromotionGate:
    ...
    def evaluate(
        ...
        if shadow_result.directional_agreement_rate is not None:
            agree_check = self._check_agreement(shadow_result.directional_agreement_rate)
    ...
    def _check_sharpe(
        ...
        if len(returns) >= 20:
            try:
                from quantcore.backtesting.stats import sharpe_ratio_with_ci

                ret_arr = np.array(returns) / 100.0  # convert pct to decimal
                _, (lower, upper) = sharpe_ratio_with_ci(ret_arr)
                value = lower
                message_suffix = f"(CI lower bound, Lo 2002: [{lower:.2f}, {upper:.2f}])"
