...
from quantcore.data.streaming.incremental_features import IncrementalFeatureEngine
...
class LiveIntradayLoop:
    ...
    def __init__(
        ...
        self._paper_mode = paper_mode or not os.getenv("USE_REAL_TRADING", "").lower() == "true"
    ...
    async def run(self) -> IntradayReport:
        ...
        if not strategies:
            logger.warning("[IntradayLoop] No intraday strategies found — nothing to trade")
        ...
        async def broker_execute(symbol: str, side: str, quantity: float, reason: str) -> dict:
            """Submit an exit order through the paper broker."""
            from quant_pod.execution.paper_broker import OrderRequest, get_paper_broker
            ...
            if fill:
                fill_tracker.update_fill(FillEvent(
            ...
            return {"fill": fill is not None, "symbol": symbol, "side": side, "reason": reason}
        ...
        from quantcore.execution.risk_gate import PreTradeRiskGate
        from quantcore.execution.smart_order_router import SmartOrderRouter

        risk_gate = PreTradeRiskGate()
        router = SmartOrderRouter(
            alpaca_broker=None, ibkr_broker=None,
            fill_tracker=fill_tracker, paper=self._paper_mode,
        )
        ...
        if self._dry_run:
            logger.info("[IntradayLoop] Dry run — skipping streaming. All components initialized.")
        ...
        if adapter is None:
            ...
            logger.info("[IntradayLoop] No streaming adapter (paper mode) — session ready")
            report.session_duration_seconds = time.monotonic() - t0
            return report
        ...
        logger.info(f"[IntradayLoop] Subscribing to {self._symbols} on {self._provider}")
        ...
        report.positions_flattened = len([
            e for e in position_manager.exit_log
    ...
    def _create_adapter(self):
        ...
        if self._provider == "alpaca":
            try:
                from quantcore.data.streaming.alpaca_stream import AlpacaStreamingAdapter
            ...
            try:
                from quantcore.data.streaming.polygon_stream import PolygonStreamingAdapter
            ...
            try:
                from quantcore.data.streaming.ibkr_stream import IBKRStreamingAdapter
        ...
        logger.warning(f"[IntradayLoop] Unknown provider '{self._provider}' — no streaming")
        return None
