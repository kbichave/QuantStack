...
def _series(n: int = 300, base: float = 2.0, noise: float = 0.1, seed: int = 42) -> pd.Series:
    np.random.seed(seed)
...
class TestRealYieldFeatures:
    ...
    def test_required_columns(self):
        ...
        result = RealYieldFeatures().compute(ry, be)
        for col in ("real_yield_10y", "breakeven_10y", "real_yield_momentum",
                     "breakeven_momentum", "growth_value_signal"):
...
class TestCreditSpreadFeatures:
    ...
    def test_regime_values(self):
        oas = _series(base=4.0, noise=0.2)
        result = CreditSpreadFeatures().compute(oas)
        assert set(result["credit_regime"].unique()).issubset({"TIGHT", "NORMAL", "WIDE"})
...
class TestCopperGoldRatio:
    ...
    def test_regime_values(self):
        ...
        au = _series(base=1800.0, noise=10.0, seed=99)
        result = CopperGoldRatio().compute(cu, au)
        assert set(result["cg_regime"].unique()).issubset({"EXPANSION", "NEUTRAL", "CONTRACTION"})
...
class TestDXYMomentum:
    ...
    def test_regime_values(self):
        dxy = _series(base=103.0, noise=0.3)
        result = DXYMomentum().compute(dxy)
        assert set(result["dxy_regime"].unique()).issubset({"STRONG", "NEUTRAL", "WEAK"})
...
class TestEquityBondCorrelation:
    ...
    def test_regime_values(self):
        ...
        bd = _close(seed=77).pct_change()
        result = EquityBondCorrelation().compute(eq, bd)
        assert set(result["eq_bond_corr_regime"].unique()).issubset({"NORMAL", "CRISIS"})
