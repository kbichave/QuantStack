...
class StreamManager:
    ...
    def __init__(
        ...
        self._adapter: Any = None  # StreamingAdapter — lazy to avoid import if not needed
    ...
    async def start(self, symbols: list[str]) -> None:
        ...
        self._consumer_tasks = [
            asyncio.create_task(
                self._consume_for_store(), name="stream_manager_store"
            ),
            asyncio.create_task(
        ...
        if api_key and secret_key:
            try:
                from quantcore.data.streaming.alpaca_stream import (
                    AlpacaStreamingAdapter,
                )
    ...
    async def add_symbols(self, symbols: list[str]) -> None:
        ...
        new_symbols = [s.upper().strip() for s in symbols if s.upper().strip() not in self._symbols]
        if not new_symbols:
            return
        ...
        if self._adapter is not None and not self._passive_mode:
            try:
                ...
                logger.warning(f"[StreamManager] Failed to subscribe {new_symbols}: {exc}")
    ...
    async def remove_symbols(self, symbols: list[str]) -> None:
        ...
        if self._adapter is not None and not self._passive_mode:
            try:
                ...
                logger.warning(f"[StreamManager] Failed to unsubscribe {to_remove}: {exc}")
    ...
    async def _warm_symbol(self, symbol: str) -> None:
        ...
        try:
            df = await asyncio.to_thread(
                self._store.load_ohlcv, symbol, Timeframe.D1
            )
            ...
            for _, row in warmup.iterrows():
                bar = BarEvent(
                    symbol=symbol,
                    timestamp=row.name if hasattr(row.name, 'tzinfo') else datetime.now(timezone.utc),
                    open=float(row["open"]),
                    high=float(row["high"]),
                    low=float(row["low"]),
