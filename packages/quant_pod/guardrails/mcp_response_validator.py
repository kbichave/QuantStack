...
class MCPResponseValidator:
    ...
    def validate_quote_response(self, response: dict[str, Any]) -> ValidationResult:
        ...
        if not isinstance(response, dict):
            result.add("response", type(response).__name__, "Expected dict, got non-dict")
            result.log()
            return result

        symbol = str(response.get("symbol", "UNKNOWN"))

        for price_field in ("price", "last", "bid", "ask", "close", "open", "high", "low"):
    ...
    def validate_ohlcv_response(
        ...
        for field_name in required:
            ...
            if val is None:
                result.add(field_name, None, f"Missing required OHLCV field: {field_name}")
                continue
            if not isinstance(val, (int, float)):
                result.add(field_name, val, "Non-numeric price")
                continue
            if not (_PRICE_MIN <= val <= _PRICE_MAX):
                result.add(field_name, val, f"Price out of bounds [{_PRICE_MIN}, {_PRICE_MAX}]")
    ...
    def validate_options_response(self, response: dict[str, Any]) -> ValidationResult:
        ...
        if iv is not None:
            if not isinstance(iv, (int, float)):
                ...
                result.add("iv", iv, f"IV out of plausible range [{_IV_MIN}, {_IV_MAX}]")
    ...
    def validate_portfolio_response(self, response: dict[str, Any]) -> ValidationResult:
        ...
        if not isinstance(positions, list):
            result.add("positions", type(positions).__name__, "Expected list of positions")
            result.log()
            return result
    ...
    def validate_generic_response(
        ...
        if response is None:
            result.add("response", None, "Tool returned None — possible tool call failure")
            result.log()
            return result
