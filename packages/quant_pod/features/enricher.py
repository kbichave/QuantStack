...
class FeatureEnricher:
    ...
    def enrich(
        ...
        if tiers is None:
            tiers = FeatureTiers(
                fundamentals=True, earnings=True, macro=True, flow=True,
            )
    ...
    def _merge_earnings(self, df: pd.DataFrame, symbol: str) -> pd.DataFrame:
        ...
        try:
            from quantcore.features.fundamental_features import EarningsFeatures
    ...
    def _merge_macro(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            from quantcore.features.economic_features import EconomicFeatureEngineer
