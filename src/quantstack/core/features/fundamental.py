...
class OperatingLeverage:
    ...
    def compute(self, financials_df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["revenue_change_pct"] = (
            (df["revenue"] - df["revenue"].shift(1)) / prior_rev * 100
        )
        df["ebitda_change_pct"] = (
            (df["ebitda"] - df["ebitda"].shift(1)) / prior_ebitda.abs() * 100
        )
...
class PiotroskiFScore:
    ...
    def compute(self, financials_df: pd.DataFrame) -> pd.DataFrame:
        ...
        current_ratio = df["current_assets"] / df["current_liabilities"].replace(
            0, np.nan
        )
        gross_margin = (df["revenue"] - df["cost_of_revenue"]) / df["revenue"].replace(
            0, np.nan
        )
        ...
        f_accrual = (cfo_over_assets > roa).astype(
            int
        ...
        f_no_dilution = (
            df["shares_outstanding"] <= df["shares_outstanding"].shift(4)
        ).astype(int)
...
class QualityMomentumComposite:
    ...
    def __init__(
        ...
        quality_short_threshold: int = 2,
        momentum_long_pct: float = 0.6,
        momentum_short_pct: float = 0.4,
    ) -> None:
    ...
    def compute(
        self, financials_df: pd.DataFrame, price_series: pd.Series
    ) -> pd.DataFrame:
        """
        Parameters
        ----------
...
class EarningsMomentumComposite:
    ...
    def compute(
        self, earnings_df: pd.DataFrame, price_series: pd.Series
    ) -> pd.DataFrame:
        """
        Parameters
        ----------
        ...
        dual_confirmation = (
            (sue_signal > 0) & (mom_zscore > 0) | (sue_signal < 0) & (mom_zscore < 0)
        ).astype(int)
