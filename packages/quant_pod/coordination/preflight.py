...
class PreflightReport:
    ...
    def summary(self) -> str:
        ...
        for c in self.checks:
            icon = "PASS" if c.passed else ("BLOCK" if c.severity == "blocker" else "WARN")
            lines.append(f"  [{icon}] {c.name}: {c.detail}")

        if self.blockers:
            lines.append("")
            lines.append(f"  {len(self.blockers)} blocker(s) must be resolved before trading.")
...
class PreflightCheck:
    ...
    def _check_database(self) -> PreflightResult:
        ...
        try:
            ...
            required = {
                "strategies", "positions", "cash_balance", "fills",
                "decision_events", "system_state", "universe",
                "screener_results", "loop_events", "loop_heartbeats",
            }
            ...
            return PreflightResult("Database tables", True, f"{len(tables)} tables present")
    ...
    def _check_kill_switch(self) -> PreflightResult:
        ...
        try:
            ...
            return PreflightResult("Kill switch", True, "No kill switch state found (OK)")
    ...
    def _check_screener(self) -> PreflightResult:
        ...
        try:
            ...
            return PreflightResult("Screener", True, "Table empty (OK for first run)", severity="warning")
    ...
    def _check_risk_limits(self) -> PreflightResult:
            ...
            try:
                ...
                if row:
                    ...
                    if price > max_per_position:
                        fractional = os.getenv("ALPACA_FRACTIONAL_SHARES", "true").lower() in ("true", "1")
                        if fractional:
                            warnings.append(
        ...
        return PreflightResult("Risk limits", True, detail, severity="warning" if warnings else "blocker")
    ...
    def _check_broker(self) -> PreflightResult:
        ...
        if ibkr_host:
            return PreflightResult("Broker", True, f"IBKR configured (host={ibkr_host})")
    ...
    def _check_options_execution(self) -> PreflightResult:
        ...
        try:
            ...
            from quant_pod.mcp.tools.options_execution import execute_options_trade
            tool_available = True
        except ImportError:
