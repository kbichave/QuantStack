    ...
    def record_entry(
        ...
        try:
            ...
            ctx.db.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                [strategy_id, symbol, regime, action, price,
    ...
    def record_exit(
        ...
        try:
            ...
            ctx.db.execute(
                ...
                WHERE id = ?
                """,
                [exit_price, realized_pnl_pct, outcome,
    ...
    def apply_learning(self, strategy_id: str) -> bool:
        ...
        try:
            ...
            if current_affinity is None:
                logger.debug(f"[OutcomeTracker] strategy {strategy_id} not found — skip")
                return False
            ...
            for regime, pnls in by_regime.items():
                ...
                new_val = max(_CLIP_MIN, min(_CLIP_MAX, current_val + _STEP * mean_weight))
