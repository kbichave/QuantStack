...
class DecisionLog:
    ...
    def __init__(
        ...
        if conn is not None:
            ...
            if db_path is None:
                db_path = os.getenv("AUDIT_LOG_DB_PATH", "~/.quant_pod/audit_log.duckdb")
...
def extract_indicator_attributions(
    ...
    if adx is not None:
        ...
        _attr("ADX", adx, signal, min((adx - 20) / 30, 1.0) if adx > 20 else 0.1, threshold=25.0)
    ...
    atr_pct = market_data_snapshot.get("ATR_pct") or market_data_snapshot.get("atr_percentile")
    if atr_pct is not None:
        regime = "high_vol" if atr_pct > 75 else ("low_vol" if atr_pct < 25 else "normal_vol")
