...
class Watchdog:
    ...
    async def run_forever(self) -> None:
        ...
        self._running = True
        logger.info(
            f"[Watchdog] Starting (interval={self._interval}s, resume_threshold={self._resume_threshold})"
        )
    ...
    def _check_kill_switch(self) -> HealthCheck:
        ...
        try:
            ...
            return HealthCheck(
                name="kill_switch", status="OK", message="Kill switch inactive"
            )
        except Exception as exc:
            return HealthCheck(
                name="kill_switch", status="WARNING", message=f"Cannot check: {exc}"
            )

    def _check_equity_snapshot(self) -> HealthCheck:
        """Check that daily equity snapshot is current (written today or last trading day)."""
        try:
            row = self._conn.execute("SELECT MAX(date) FROM daily_equity").fetchone()
            ...
            return HealthCheck(
                name="equity_snapshot", status="WARNING", message=f"Cannot check: {exc}"
            )
