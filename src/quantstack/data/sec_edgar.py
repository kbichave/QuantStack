...
class SECEdgarClient:
    ...
    def get_recent_filings(
        ...
        try:
            ...
            logger.error(
                f"[EDGAR] Could not fetch filings for {ticker} (CIK {cik}): {exc}"
            )
            return []
    ...
    def fetch_filing_text(
        self, filing: FilingMetadata, max_chars: int = 200_000
    ) -> str:
        """
...
class EdgarSignalBuilder:
    ...
    def _build_one(
        ...
        weighted_signal = (
            sum(s.signal * w for s, w in zip(scored, weights, strict=False))
            / total_weight
        )
