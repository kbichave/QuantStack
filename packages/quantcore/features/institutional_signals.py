...
class LSVHerding:
    ...
    def compute(self, ownership_df: pd.DataFrame) -> pd.DataFrame:
        ...
        expected = df["fraction_buying"].rolling(
            window=self.rolling_window, min_periods=2
        ).mean()
        ...
        return df[[
            "period_end", "fraction_buying", "expected_buying",
...
class InstitutionalConcentration:
    ...
    def compute(self, ownership_df: pd.DataFrame) -> pd.DataFrame:
        ...
        return df[[
            "period_end", "total_holders", "holder_change",
            "holder_change_pct", "institutionalizing", "de_institutionalizing",
        ]]
