...
class EtradeBroker:
    ...
    def execute(self, req: OrderRequest) -> Fill:
        ...
        if not self._auth.is_authenticated():
            return self._reject(req, "eTrade not authenticated — call /etrade/auth first")
        ...
        try:
            ...
            for o in orders[:limit]:
                fill = Fill(
                    ...
                    side="buy" if (o.legs and "BUY" in o.legs[0].order_action.value) else "sell",
    ...
    def _poll_for_fill(self, order_id: str):
        ...
        """
        from quant_pod.tools.etrade.models import OrderStatus
    ...
    def _to_fill(self, req: OrderRequest, order, live_price: float, commission: float) -> Fill:
        ...
        slippage_bps = abs(fill_price - ref_price) / ref_price * 10_000 if ref_price > 0 else 0.0
    ...
    def _update_portfolio(self, fill: Fill, req: OrderRequest) -> None:
        ...
        cash_delta = -direction * fill.filled_quantity * fill.fill_price - fill.commission
    ...
    def _reconcile_on_startup(self) -> None:
        ...
        try:
            ...
            etrade_cash = float(balance.cash_available_for_investment or balance.net_cash or 0)
    ...
    def _register_kill_switch_closer(self) -> None:
        ...
        def close_all_etrade_positions() -> None:
            ...
            try:
                open_orders = self._client.get_orders(self._account_id_key, status="OPEN")
                for order in open_orders:
                    try:
                        self._client.cancel_order(self._account_id_key, str(order.order_id))
                        logger.info(f"[ETRADE] Cancelled order {order.order_id}")
                    except Exception as e:
                        logger.error(f"[ETRADE] Cancel order {order.order_id} failed: {e}")
            except Exception as e:
            ...
            try:
                etrade_positions = self._client.get_positions(self._account_id_key)
                for ep in etrade_positions:
                    if ep.quantity and ep.quantity > 0 and ep.symbol and len(ep.symbol) <= 5:
                        try:
                            req = OrderRequest(
                                symbol=ep.symbol,
                                side="sell",
                            ...
                            logger.error(f"[ETRADE] Close position {ep.symbol} failed: {e}")
            except Exception as e:
