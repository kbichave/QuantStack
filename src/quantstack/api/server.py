...
from quantstack.monitoring.metrics import (
...
from quantstack.tools.options_flow_tools import OptionsFlowClient
...
def get_audit_attribution(event_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "derived": len(event.indicator_attributions)
        == 0,  # True if we derived vs. stored
    }
...
def get_monthly_returns() -> dict[str, Any]:
    ...
    try:
        ...
        for row in curve:
            ...
            monthly.setdefault(y, {})[m] = (
                monthly.get(y, {}).get(m, 0) + row["daily_return_pct"]
            )
...
def get_benchmark_comparison(
    ...
    try:
        ...
        comparisons = tracker.get_comparison(
