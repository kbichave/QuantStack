...
class ResearchContext:
    ...
    def get_experiment_history(self, days: int = 30, limit: int = 20) -> list[dict]:
        ...
        try:
            ...
            return [
                {
                    "id": r[0], "symbol": r[1], "model_type": r[2],
                    "feature_tiers": _safe_json(r[3]),
                    "test_auc": r[4], "cv_auc_mean": r[5],
                    "top_features": _safe_json(r[6]),
                    "verdict": r[7], "failure_analysis": r[8],
                    "hypothesis_id": r[9], "date": str(r[10]),
                }
                for r in rows
            ]
    ...
    def get_strategy_pnl_summary(self, days: int = 30) -> list[dict]:
        ...
        try:
            ...
            return [
                {
                    "strategy_id": r[0], "total_pnl": round(r[1], 2),
                    "total_trades": r[2], "wins": r[3], "losses": r[4],
                    "win_rate": round(r[3] / (r[3] + r[4]), 3) if (r[3] + r[4]) > 0 else 0,
                    "trading_days": r[5],
                }
                for r in rows
            ]
    ...
    def get_active_investigations(self) -> list[dict]:
        ...
        try:
            ...
            return [
                {
                    "id": r[0], "thesis": r[1], "status": r[2],
                    "priority": r[3], "experiments_run": r[4],
                    "best_oos_sharpe": r[5], "last_result": r[6],
                    "next_steps": r[7],
    ...
    def get_regime_summary(self) -> dict:
        ...
        try:
            from quant_pod.signal_engine.collectors.regime import _rule_based_regime
    ...
    def get_model_status(self) -> list[dict]:
        ...
        try:
            ...
            for r in rows:
                if r[0] not in seen:
                    seen.add(r[0])
                    age_days = (date.today() - r[4].date()).days if hasattr(r[4], 'date') else 0
                    results.append({
                        "symbol": r[0], "model_type": r[1],
                        "test_auc": r[2], "cv_auc_mean": r[3],
                        "age_days": age_days,
                        "stale": age_days > 30,
