...
class RouteContext:
    ...
    force_size: str | None = (
        None  # Override position size (e.g., "quarter" for extreme vol)
    )
...
class DecisionRouter:
    ...
    def route(
        ...
        if bias == "neutral" and conviction < _MIN_CONVICTION:
            return (
                DecisionPath.SKIP,
                f"neutral bias with low conviction ({conviction:.0%})",
                ctx,
            )
        ...
        if not matching_strategies:
            return (
                DecisionPath.SKIP,
                f"no active strategy matches regime '{trend_regime}'",
                ctx,
            )
        ...
        if last_regime:
            ...
            if (
                ...
                and prev_trend != "unknown"
                and curr_trend != "unknown"
            ):
...
def _count_strategy_disagreements(strategies: list[dict], brief_bias: str) -> int:
    ...
    for strat in strategies:
        ...
        is_mean_reversion = any(
            kw in style for kw in ("mean_rev", "rsimr", "reversion", "mr")
        )
