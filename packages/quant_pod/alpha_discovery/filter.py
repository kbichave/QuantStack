...
class FilterResult:
    ...
    passed: bool
    stage_rejected: str | None  # "is_screen", "oos_validation", "portfolio_fit", or None
    ...
    # Stage 3 fields
    portfolio_correlation: float = 0.0        # highest correlation found against existing strategies
    ...
    try:
        ...
        for strategy_id, er_raw, xr_raw, params_raw in rows:
            try:
                ...
                logger.debug(f"[PortfolioFitCheck] strategy {strategy_id} signal gen failed: {exc}")
                continue
