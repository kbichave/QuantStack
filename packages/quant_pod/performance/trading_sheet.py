...
class TradingSheet:
    ...
    def to_markdown(self) -> str:
        ...
        if self.hmm_stability is not None:
            lines.append(f"- HMM stability: {self.hmm_stability:.0%}, expected duration: {self.hmm_expected_duration:.0f} bars")
        ...
        lines.extend([
            ...
            f"- MACD hist: {self.macd_hist:+.3f}" if self.macd_hist is not None else "- MACD hist: N/A",
            f"- ATR(14): ${self.atr_14:.2f}" if self.atr_14 else "- ATR(14): N/A",
            f"- SMA 20/50/200: ${self.sma_20:.2f} / ${self.sma_50:.2f} / ${self.sma_200:.2f}" if self.sma_200 else "- SMAs: N/A",
            f"- Bollinger: ${self.bb_lower:.2f} — ${self.bb_upper:.2f}" if self.bb_upper else "- Bollinger: N/A",
        ...
        if self.opt_gex is not None:
            lines.extend([
                ...
                f"- Gamma flip: ${self.opt_gamma_flip:.2f} (spot {'above' if self.opt_above_gamma_flip else 'below'})" if self.opt_gamma_flip else "- Gamma flip: N/A",
                f"- DEX: {self.opt_dex:,.0f}" if self.opt_dex else "- DEX: N/A",
                f"- Max pain: ${self.opt_max_pain:.2f}" if self.opt_max_pain else "- Max pain: N/A",
                f"- IV skew: {self.opt_iv_skew:.4f}" if self.opt_iv_skew else "- IV skew: N/A",
                f"- VRP: {self.opt_vrp:.4f}" if self.opt_vrp else "- VRP: N/A",
                f"- Put/Call OI: {self.opt_put_oi:,}/{self.opt_call_oi:,}" if self.opt_call_oi else "- Put/Call OI: N/A",
            ])
        ...
        lines.extend([
            ...
            f"- ML: {self.ml_direction} (prob={self.ml_probability:.2f}, conf={self.ml_confidence:.0%})" if self.ml_probability else "- ML: no model trained",
        ])
        ...
        lines.extend([
            ...
            f"- Take profit: ${self.take_profit:.2f}" if self.take_profit else "- Take profit: N/A",
...
class TradingSheetGenerator:
    ...
    async def generate(
        ...
        try:
            ...
            if df is not None and len(df) >= 200:
                ...
                sheet.rsi_14 = float(100 - 100 / (1 + rs.iloc[-1])) if rs.iloc[-1] == rs.iloc[-1] else None
    ...
    def _derive_action(self, sheet: TradingSheet) -> str:
        ...
        if sheet.consensus_bias in ("bullish", "strong_bullish") and sheet.consensus_conviction >= 0.5:
            return "BUY"
        if sheet.consensus_bias in ("bearish", "strong_bearish") and sheet.consensus_conviction >= 0.5:
            return "SELL"
        return "HOLD"
    ...
    def _derive_rationale(self, sheet: TradingSheet) -> str:
        ...
        if sheet.opt_gex is not None:
            parts.append(f"GEX {'positive (dampening)' if sheet.opt_gex > 0 else 'negative (amplifying)'}")
