...
class MeanVarianceOptimizer:
    ...
    def optimize(
        ...
        symbols = [
            s for s in signals if s in cov_matrix.index and s in cov_matrix.columns
        ]
        if not symbols:
            raise ValueError(
        ...
        weighted_avg_vol = (
            float(w_arr @ individual_vols) if np.all(w_arr >= 0) else port_vol
        )
        ...
        if current_weights is not None:
            required_trades = {
                s: target_dict[s] - current_weights.get(s, 0.0) for s in symbols
            }
        ...
        result._risk_contributions = self._compute_risk_contributions(
            w_arr, sigma, symbols
        )
        ...
        if constraints.sector_map:
            ...
            for _sector, indices in sectors.items():
                ...
                def sector_constraint(
                    w: np.ndarray, idxs: list[int] = sector_indices
                ) -> float:
...
def trades_from_weights(
    ...
    return sorted(
        trades, key=lambda t: (0 if t["side"] == "sell" else 1, -t["notional"])
    )
