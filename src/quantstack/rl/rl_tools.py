...
from quantstack.rl.config import RLProductionConfig, get_rl_config
...
class RLPositionSizeInput(BaseModel):
    ...
    current_drawdown: float = Field(
        default=0.0,
        ge=0.0,
        description="Current portfolio drawdown fraction (e.g. 0.05)",
    )
...
class RLExecutionStrategyInput(BaseModel):
    ...
    urgency: str = Field(
        default="normal", description="Urgency level: low, normal, high"
    )
...
class RLAlphaWeightInput(BaseModel):
    regime: str = Field(
        ..., description="Current market regime (e.g. trending_up, ranging)"
    )
...
class RLExecutionStrategyTool(BaseTool):
    ...
    def _run(
        ...
        strategy_label, order_fraction = self._STRATEGY_MAP.get(
            action_idx, ("BALANCED", 0.25)
        )
    ...
    def _load_agent(self, cfg: RLProductionConfig) -> Any | None:
        ...
        try:
            ...
            object.__setattr__(self, "_agent", agent)
            logger.info(
                "[RLExecutionStrategyTool] Execution agent loaded from checkpoint."
            )
            return agent
        except Exception as exc:
            logger.warning(
                f"[RLExecutionStrategyTool] Failed to load checkpoint: {exc}"
            )
            return None
...
class RLAlphaWeightTool(BaseTool):
    ...
    def _run(
        ...
        if not cfg.enable_meta_rl:
            ...
            return json.dumps(
                {
                    "selected_alpha": (
                        competing_signals[0] if competing_signals else "NONE"
                    ),
        ...
        alpha_histories: dict[str, list[float]] = {
            name: [] for name in competing_signals
        }
    ...
    def _load_agent(
        ...
        try:
            ...
            logger.info(
                "[RLAlphaWeightTool] Alpha selection agent loaded from checkpoint."
            )
            return agent
        except Exception as exc:
...
def get_rl_tools(config: RLProductionConfig | None = None) -> list[BaseTool]:
    ...
    logger.debug(
        f"[RL] {len(tools)} RL tools registered (shadow={cfg.shadow_mode_enabled})."
    )
    return tools
