...
def _analyze_entry_patterns(signals: list[dict]) -> dict[str, Any]:
    ...
    return {
        ...
        "direction_bias": (
            "long" if long_pct > 65 else ("short" if long_pct < 35 else "mixed")
        ),
...
def _analyze_sizing_patterns(signals: list[dict]) -> dict[str, Any]:
    ...
    big_win_rate = sum(1 for s in big_trades if s["is_winner"]) / max(
        1, len(big_trades)
    )
    small_win_rate = sum(1 for s in small_trades if s["is_winner"]) / max(
        1, len(small_trades)
    )
