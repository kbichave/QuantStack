...
class AutoPromoter:
    ...
    def is_enabled(self) -> bool:
        ...
        return os.getenv("AUTO_PROMOTE_ENABLED", "false").lower() in ("true", "1", "yes")
    ...
    def evaluate_all(self) -> list[PromotionDecision]:
        ...
        if not self.is_enabled():
            logger.info("[AutoPromoter] Disabled (set AUTO_PROMOTE_ENABLED=true to enable)")
            return []
    ...
    def get_ramp_scale(self, strategy_id: str) -> float:
        ...
        days_live = (datetime.now(timezone.utc) - promotion_date.replace(tzinfo=timezone.utc)).days
    ...
    def _evaluate_one(
        ...
        if not isinstance(updated_at, datetime):
            return PromotionDecision(
                ...
                decision="hold", reason="Cannot determine age",
            )
        ...
        if not checks["min_age"]:
            return PromotionDecision(
                ...
                evidence=evidence, criteria_results=checks,
            )
        ...
        pnl_pcts = [o["realized_pnl_pct"] for o in outcomes if o.get("realized_pnl_pct") is not None]
        if not pnl_pcts:
            return PromotionDecision(
                strategy_id=strategy_id, name=name,
                decision="hold", reason="No realized P&L data",
                evidence=evidence, criteria_results=checks,
            )
        ...
        if bt_sharpe and bt_sharpe > 0:
            ...
            checks["max_degradation"] = degradation <= criteria.max_degradation_vs_backtest
        else:
    ...
    def _get_backtest_sharpe(self, bt_summary_raw: str | None) -> float | None:
        ...
        if not bt_summary_raw:
            return None
        try:
            summary = json.loads(bt_summary_raw) if isinstance(bt_summary_raw, str) else bt_summary_raw
