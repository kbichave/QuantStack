...
Usage:
    from quantstack.risk.portfolio_risk import PortfolioRiskAnalyzer
...
class PortfolioRiskAnalyzer:
    ...
    def analyze(
        ...
        if returns_df is not None and len(returns_df) >= _CORR_WINDOW:
            try:
                ...
                logger.warning(
                    f"[PORTFOLIO_RISK] Correlation computation failed: {exc}"
                )
    ...
    def check_correlation_risk(
        ...
        """
        held_symbols = [
            p["symbol"] for p in positions if p["symbol"] != proposed_symbol
        ]
    ...
    def compute_factor_exposure(self, positions: list[dict]) -> FactorExposure:
        ...
        sector_hhi = (
            sum(w**2 for w in sector_weights.values()) if sector_weights else 0.0
        )
    ...
    def compute_concentration_risk(self, positions: list[dict]) -> ConcentrationReport:
        ...
        sector_weights = self._compute_sector_weights(positions, total_value)
        sector_breach = any(
            w > _SECTOR_BREACH_THRESHOLD for w in sector_weights.values()
        )
    ...
    def _compute_risk_score(
        ...
        if concentration.sector_breach:
            ...
            warnings.append(
                f"Sector concentration breach: {', '.join(breached)} exceed 30%."
            )
            recommendations.append(
