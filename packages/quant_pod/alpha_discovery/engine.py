...
from quant_pod.alpha_discovery.hypothesis_agent import HypothesisAgent
...
class AlphaDiscoveryEngine:
    ...
    def run(self, symbols: list[str] | None = None) -> DiscoveryResult:
        ...
        logger.info(
            f"[AlphaDiscoveryEngine] run={run_id} starting "
    ...
    def _process_symbol(self, symbol: str, result: DiscoveryResult) -> None:
        ...
        try:
            ...
            from quant_pod.alpha_discovery.filter import CandidateFilter
            from quant_pod.alpha_discovery.registrar import StrategyRegistrar
            from quant_pod.alpha_discovery.search_space import ParameterGrid
            ...
            from quant_pod.alpha_discovery.filter import IS_MIN_TRADES
            ...
            for template_name, template_rules in templates:
                ...
                for params in grid:
                    ...
                    filter_result = filt.apply(spec, price_data, n_trials=grid.total_combinations)
            ...
            if self._enable_gp and len(price_data) >= 252:
                try:
                    from quant_pod.alpha_discovery.grammar_gp import GrammarGP
        ...
        logger.debug(
            f"[AlphaDiscoveryEngine] {symbol}: done in "
    ...
    def _load_price_data(self, symbol: str) -> Any:
        ...
        try:
            ...
            logger.warning(f"[AlphaDiscoveryEngine] {symbol}: could not load data — {exc}")
            return None

    def _detect_regime(self, price_data: Any) -> dict[str, Any]:
        """Classify regime from loaded OHLCV (no re-fetch)."""
        try:
            from quantcore.hierarchy.regime.regime_classifier import WeeklyRegimeClassifier
    ...
    def _get_templates(
        ...
        """
        from quant_pod.alpha_discovery.search_space import get_templates_for_regime
...
def _get_rules_for_template(template_name: str) -> tuple[list[dict], list[dict]]:
    ...
    if template_name == "rsi_mean_reversion":
        entry_rules = [
            ...
            {"indicator": "rsi", "condition": "crosses_below", "value": 30,
             "type": "prerequisite", "_param_value": "rsi_oversold"},
            {"indicator": "sma_fast", "condition": "above", "value": "sma_slow",
             "type": "confirmation"},
        ]
        exit_rules = [
            {"indicator": "rsi", "condition": "crosses_above", "value": 70,
             "_param_value": "rsi_overbought"},
            {"type": "stop_loss", "atr_multiple": 2.0, "_param_atr_multiple": "stop_loss_atr"},
        ]

    elif template_name == "trend_momentum":
        entry_rules = [
            {"indicator": "sma_crossover", "condition": "crosses_above", "value": 0,
             "type": "prerequisite"},
            {"indicator": "adx", "condition": "above", "value": 25,
             "_param_value": "adx_threshold", "type": "confirmation"},
        ]
        exit_rules = [
        ...
        entry_rules = [
            {"indicator": "breakout", "condition": "above", "value": 0,
             "type": "prerequisite"},
            {"indicator": "adx", "condition": "above", "value": 20,
             "type": "confirmation"},
        ]
        exit_rules = [
            {"type": "stop_loss", "atr_multiple": 1.5, "_param_atr_multiple": "stop_loss_atr"},
            {"type": "take_profit", "atr_multiple": 3.0},
        ]

    elif template_name == "mean_reversion_bollinger":
        entry_rules = [
            {"indicator": "bb_pct", "condition": "below", "value": 0.1,
             "type": "prerequisite"},
            {"indicator": "rsi", "condition": "below", "value": 35,
             "_param_value": "rsi_oversold", "type": "confirmation"},
        ]
        exit_rules = [
...
def _resolve_rule_params(
    rules: list[dict], params: dict[str, Any]
) -> list[dict]:
    """
    ...
    try:
        ...
        try:
            candidates = loop.run_until_complete(
                agent.generate(symbol=symbol, regime=regime, signal_brief_summary=brief_summary)
            )
        finally:
            loop.close()
        ...
        logger.debug(f"[AlphaDiscoveryEngine] {symbol}: _run_hypothesis_agent_sync failed: {exc}")
        return []
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="AlphaDiscoveryEngine — overnight strategy discovery")
    parser.add_argument("--symbols", nargs="+", default=None, help="Symbols to search")
    parser.add_argument("--dry-run", action="store_true", help="Run pipeline, skip DB writes")
    parser.add_argument("--no-gp", action="store_true", help="Disable GP evolution (grid search + hypothesis only)")
