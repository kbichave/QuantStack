    ...
    return pd.DataFrame(
        {
            ...
            "commercial_long":      (200_000 + np.random.randint(-10_000, 10_000, 156)).astype(float),
            "commercial_short":     (230_000 + np.random.randint(-10_000, 10_000, 156)).astype(float),
            "nonreportable_long":   (50_000 + np.random.randint(-5_000, 5_000, 156)).astype(float),
            "nonreportable_short":  (50_000 + np.random.randint(-5_000, 5_000, 156)).astype(float),
...
class TestEquityCarry:
    ...
    def test_expected_columns(self, financials):
        result = EquityCarry().compute(financials)
        assert {
            "ttm_dividends", "ttm_buybacks", "total_return_to_shareholders",
            "dividend_yield", "buyback_yield", "equity_carry", "carry_high",
...
class TestFuturesBasis:
    ...
    def test_expected_columns(self, futures_spot):
        ...
        result = FuturesBasis().compute(futures, spot)
        assert {
            "basis_pct", "basis_roll_mean", "basis_roll_std", "basis_zscore",
            "contango", "backwardation", "basis_extreme_high", "basis_extreme_low",
        }.issubset(set(result.columns))
    ...
    def test_contango_backwardation_mutually_exclusive(self, futures_spot):
        futures, spot = futures_spot
        result = FuturesBasis().compute(futures, spot)
        simultaneous = ((result["contango"] == 1) & (result["backwardation"] == 1)).sum()
...
class TestCOTSignals:
    ...
    def test_expected_columns(self, cot_data):
        result = COTSignals().compute(cot_data)
        assert {
            "nc_net", "nc_net_pct", "nc_zscore",
            "nc_extreme_long", "nc_extreme_short",
            "comm_net", "comm_net_pct", "sr_net", "cot_sentiment",
        }.issubset(set(result.columns))
