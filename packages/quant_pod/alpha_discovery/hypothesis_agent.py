...
_VALID_INDICATORS = frozenset({
    "rsi", "macd_hist", "adx_14", "bb_pct",
    "sma_fast", "sma_slow", "sma_crossover", "breakout",
})
    ...
    async def generate(
        ...
        try:
            return await asyncio.wait_for(
                asyncio.to_thread(self._generate_sync, symbol, regime, signal_brief_summary),
                timeout=_TIMEOUT,
            )
        except asyncio.TimeoutError:
            logger.debug(f"[HypothesisAgent] {symbol}: timed out after {_TIMEOUT}s — no candidates")
            return []
