...
class EarningsSurpriseSignals:
    ...
    def compute(self, earnings_df: pd.DataFrame) -> pd.DataFrame:
        ...
        surprise_std = (
            df["eps_surprise"].rolling(window=self.sue_lookback, min_periods=2).std()
        )
...
class AnalystRevisionSignals:
    ...
    def compute(self, estimates_df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["revision_momentum"] = (
            (df["consensus_eps"] - df["prior_consensus_eps"]) / prior_safe * 100
        )
...
class EarningsImpliedMove:
    ...
    def compute(self, earnings_df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["historical_iv_bias"] = (
            df["move_surprise"].rolling(window=4, min_periods=2).mean()
        )
