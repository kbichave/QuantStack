...
from quantstack.rl.base import Action, Reward, State
from quantstack.rl.config import RLProductionConfig
...
class PostTradeRLAdapter:
    ...
    def process_trade_outcome(
        ...
        try:
            ...
            logger.warning(
                f"[RL Adapter] process_trade_outcome failed (non-fatal): {exc}"
            )
    ...
    def _record_shadow(
        ...
        try:
            ...
            logger.debug(
                f"[RL Adapter] shadow record_outcome failed (non-fatal): {exc}"
            )
    ...
    def _get_sizing_trainer(self) -> Any | None:
        ...
        try:
            ...
            from quantstack.rl.training import OnlineRLTrainer, TrainingConfig
    ...
    def _get_execution_trainer(self) -> Any | None:
        ...
        try:
            ...
            logger.debug(
                f"[RL Adapter] Execution trainer init failed (non-fatal): {exc}"
            )
        return self._execution_trainer
