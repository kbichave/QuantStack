# {SYMBOL} Research & Trading Memory

> Per-ticker memory file. Updated by research and trading loops.
> Read at START of any session involving this symbol.

## Fundamental Snapshot

_Last updated: {date}_

| Metric | Value | Notes |
|--------|-------|-------|
| Revenue (TTM) | | |
| Revenue Growth YoY | | |
| Gross Margin | | |
| Operating Margin | | |
| FCF (TTM) | | |
| FCF Yield | | |
| P/E | | |
| P/S | | |
| Net Cash/Debt | | |
| Shares Outstanding | | |
| SBC (% of revenue) | | |

## Price Action

_Last updated: {date}_

| Level | Value |
|-------|-------|
| Current Price | |
| SMA 20 | |
| SMA 50 | |
| SMA 200 | |
| Key Support | |
| Key Resistance | |
| 52w High | |
| 52w Low | |

## Investment Thesis

| Thesis Type | Fit | Evidence |
|-------------|-----|----------|
| Value | | |
| Quality-Growth | | |
| Dividend | | |
| Sector Rotation | | |
| Earnings Catalyst | | |

**Primary thesis**: _describe_
**Conviction**: _X%_
**Catalyst timeline**: _describe_

## Evidence Map (from breadth-first scan)

| Category | Key Findings | Tier | Direction |
|----------|-------------|------|-----------|
| Regime | | tier_4 | |
| Technicals | | tier_1 | |
| Fundamentals | | tier_4 | |
| Insider/Institutional | | tier_3 | |
| Flow/Sentiment | | tier_2 | |
| Macro | | tier_4 | |
| Volume/Liquidity | | tier_2 | |
| Cross-domain | | context | |

**Convergence**: X/8 bullish | Y/8 bearish | Tier-3+ aligned: Z

## Strategies

| ID | Name | Type | Backtest | WF OOS | ML AUC | Status |
|----|------|------|---------|--------|--------|--------|

## Alerts

| Date | Action | Entry | Stop | Target | Confidence | Status |
|------|--------|-------|------|--------|------------|--------|

## ML Models

| Date | Model | Features | AUC (OOS) | IS-OOS Gap | Verdict |
|------|-------|----------|-----------|------------|---------|

## Research Log

### Iteration N (date)
_What was tried, what worked, what failed, what to do next_

## Lessons (ticker-specific)
_Patterns, gotchas, data quirks specific to this symbol_
