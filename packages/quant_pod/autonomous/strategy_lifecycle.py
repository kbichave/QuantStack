...
class StrategyLifecycle:
    ...
    async def run_weekly(self) -> LifecycleReport:
        ...
        for regime in gaps:
            templates = [t for t in _STRATEGY_TEMPLATES if regime in t["regime_affinity"]]
            if not templates:
        ...
        try:
            ...
            for row in rows:
                ...
                if affinity:
                    import json
                    try:
                        regimes = json.loads(affinity) if isinstance(affinity, str) else affinity
    ...
    async def _evaluate_candidate(self, template: dict, target_regime: str) -> bool:
        ...
        try:
            self._conn.execute(
                ...
                """,
                [
                    strategy_name, strategy_name, template["description"],
            ...
            logger.debug(f"[Lifecycle] Strategy {strategy_name} may already exist: {exc}")
            return False
    ...
    async def _run_walkforward(self, strategy_id: str, symbol: str) -> dict | None:
        ...
        try:
            from quant_pod.mcp.tools.backtesting import run_walkforward
            ...
            logger.debug(f"[Lifecycle] Walk-forward failed for {strategy_id}/{symbol}: {exc}")
            return None
        ...
        try:
            ...
            for row in rows:
                ...
                if backtest_json:
                    import json
                    try:
                        bt = json.loads(backtest_json) if isinstance(backtest_json, str) else backtest_json
