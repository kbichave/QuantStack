...
class MicrostructurePipeline:
    ...
    async def _on_features_callback(self, features) -> None:
        ...
        try:
            ...
            self._stats.exceptions += 1
            logger.warning(
                f"[MicroPipeline] Signal evaluation error for {features.symbol}: {_e}"
            )
            return
        ...
        try:
            ...
            if result.status == "rejected":
                logger.debug(
                    f"[MicroPipeline] SOR REJECTED {order.symbol}: {result.reject_reason}"
                )
                return
            ...
            self._stats.exceptions += 1
            logger.error(
                f"[MicroPipeline] Unexpected routing error for {order.symbol}: {_e}"
            )
