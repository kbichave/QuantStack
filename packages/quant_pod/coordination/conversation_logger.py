    ...
    def log_agent_report(
        ...
        try:
            self._conn.execute(
                ...
                """,
                [
                    cid, self._session_id, iteration,
                    agent_name, symbol, strategy_id,
                    content, summary, datetime.now(),
    ...
    def log_pm_decision(
        ...
        try:
            self._conn.execute(
                ...
                """,
                [cid, self._session_id, iteration, symbol, content, summary, datetime.now()],
            )
        except Exception as exc:
            logger.debug(f"[ConvLogger] DB insert failed: {exc}")
