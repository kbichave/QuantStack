...
Usage:
    from quantcore.execution.algo_selector import select_algo
...
def select_algo(
    ...
    if urgency not in _BUFFER_FACTORS:
        logger.warning(f"[AlgoSelector] Unknown urgency '{urgency}', defaulting to 'normal'")
        urgency = "normal"
    ...
    if tca_forecast:
        ...
        expected_slippage = tca_forecast.market_impact_bps + tca_forecast.spread_cost_bps
    ...
    if urgency == "stop_loss":
        ...
        logger.debug(f"[AlgoSelector] {symbol}: LIMIT override — VIX={vix:.1f} > {_VIX_HIGH_THRESHOLD}")
        ...
        logger.debug(f"[AlgoSelector] {symbol}: LIMIT override — ADV={adv:,.0f} < {_LOW_LIQUIDITY_ADV:,}")
