...
class MacroCalendarGenerator:
    ...
    def build_calendar(
        ...
        """
        from quantcore.microstructure.events import EventCalendar
        ...
        for year in range(start_year, end_year + 1):
            ...
            calendar.add_cpi_dates([datetime.combine(d, time(8, 30)) for d in cpi_dates])

            # NFP
            nfp_dates = _generate_nfp_dates(year)
            calendar.add_nfp_dates([datetime.combine(d, time(8, 30)) for d in nfp_dates])
    ...
    def fetch_economic_history(
        ...
        indicators = indicators or [
            "CPI", "FEDERAL_FUNDS_RATE", "UNEMPLOYMENT",
            "NONFARM_PAYROLL", "REAL_GDP", "INFLATION", "RETAIL_SALES",
        ]

        api_key = os.environ.get("ALPHA_VANTAGE_API_KEY", "")
        if not api_key:
            logger.warning("[MacroCalendar] No ALPHA_VANTAGE_API_KEY — skipping economic fetch")
            return {}
        ...
        for indicator in indicators:
            try:
                ...
                for idx, row in df.iterrows():
                    try:
                        conn.execute(
                            ...
                            """,
                            [indicator, idx.date() if hasattr(idx, 'date') else idx, row["value"]],
                        )
                        rows_inserted += 1
                    except Exception:
                        pass
                ...
                logger.debug(f"[MacroCalendar] {indicator}: {rows_inserted} rows stored")
