...
class KnowledgeStoreRLBridge:
    ...
    def get_signal_history(
        ...
        for direction, confidence, signal_type in rows:
            ...
            rl_direction = (
                direction if direction in ("LONG", "SHORT", "NEUTRAL") else "NEUTRAL"
            )
            signals.append(
                TradingSignal(
    ...
    def _enforce_rate_limit(self) -> None:
        ...
        self._av_call_timestamps = [
            t
            for t in self._av_call_timestamps
            if now - t < self._ALPHAVANTAGE_RATE_LIMIT_WINDOW_S
        ]
        if len(self._av_call_timestamps) >= self._ALPHAVANTAGE_RATE_LIMIT_CALLS:
            wait = self._ALPHAVANTAGE_RATE_LIMIT_WINDOW_S - (
                now - self._av_call_timestamps[0]
            )
            if wait > 0:
