...
Replaces the static regime-strategy matrix (CLAUDE.md S7) with an allocation
engine that:
...
from quantstack.allocation.allocation import _match_regime, compute_allocation as static_alloc
    ...
    for sid, raw_weight in hrp_weights.items():
        ...
        allocations.append(
            DynamicAllocation(
                ...
                mode=mode,
                reasoning=(
                    f"HRP weight {raw_weight:.3f} x regime {strat['regime_score']:.2f} "
                    f"x (1 - corr_penalty {penalty:.2f}) = {adjusted_weight:.3f}. "
...
def _compute_correlation_penalties(
    ...
    Penalty = max pairwise correlation above threshold x 0.5.
