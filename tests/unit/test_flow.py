...
def ohlcv():
    ...
    volume = pd.Series(
        np.random.randint(500_000, 2_000_000, 100).astype(float), index=dates
    )
    return pd.DataFrame(
...
class TestCumulativeVolumeDelta:
    ...
    def test_buy_sell_sum_to_volume(self, ohlcv):
        ...
        pd.testing.assert_series_equal(
            total, ohlcv["volume"], check_names=False, atol=1e-6
        )
    ...
    def test_signed_volume_bypass(self, ohlcv):
        ...
        assert (
            result["cvd"] > 0
        ).all()  # all positive signed volume → CVD always positive
...
class TestHawkesIntensity:
    ...
    def test_expected_columns(self, ohlcv):
        ...
        assert {"event", "intensity", "intensity_high", "excited"}.issubset(
            set(result.columns)
        )
    ...
    def test_intensity_decays_after_event(self):
        ...
        result = HawkesIntensity(
            decay=0.5, excitation=1.0, baseline=0.1, event_threshold=1.5, event_window=5
        ).compute(high, low, close, vol)
...
def ohlcv_footprint():
    ...
    open_ = close.shift(1).fillna(close.iloc[0])
    volume = pd.Series(
        np.random.randint(100_000, 500_000, 50).astype(float), index=dates
    )
...
class TestFootprintApproximation:
    ...
    def test_stacked_bull_requires_n_consecutive(self):
        ...
        close = pd.Series(
            [100.0, 101.0, 102.0, 103.0, 104.0, 103.5, 103.0, 103.5, 104.0, 104.5],
            index=dates,
        )
        ...
        result = FootprintApproximation(stack_n=3).compute(
    ...
    def test_poc_price_within_bar_range(self, ohlcv_footprint):
        ...
        open_, high, low, close, vol = ohlcv_footprint
        result = FootprintApproximation(lookback=10).compute(
    ...
    def test_poc_nan_before_lookback(self, ohlcv_footprint):
        ...
        open_, high, low, close, vol = ohlcv_footprint
        result = FootprintApproximation(lookback=15).compute(
            open_, high, low, close, vol
        )
    ...
    def test_zero_volume_bar_handled(self):
        ...
        result = FootprintApproximation(lookback=3).compute(
            open_, high, low, close, volume
        )
