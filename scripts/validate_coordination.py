...
def main() -> int:
    ...
    if fd_key:
        ...
        _fail("FINANCIAL_DATASETS_API_KEY not set — universe refresh will skip equities")
        errors.append("No FD.ai API key")

    # ── Step 1: Fresh DB + migrations ────────────────────────────────────
    _header("Step 1: Fresh database + migrations")
    from quant_pod.db import open_db, reset_connection, run_migrations
    ...
    expected_new = {"universe", "screener_results", "loop_events", "loop_cursors", "loop_heartbeats"}
    present = expected_new & set(tables)
    missing = expected_new - set(tables)

    if not missing:
        _ok(f"{len(tables)} tables created (including {len(present)} new coordination tables)")
    else:
    ...
    for source in ("sp500", "nasdaq100", "etf_liquid"):
        syms = registry.get_active_symbols(
            __import__("quant_pod.coordination.universe_registry", fromlist=["UniverseSource"]).UniverseSource(source)
        )
    ...
    if fd_key:
        from quantcore.data.cache_warmer import CacheWarmer
        ...
        if not sample_symbols:
            sample_symbols = ["SPY", "QQQ", "AAPL", "MSFT", "NVDA", "XOM", "JPM", "GLD", "TLT", "IWM"]
                ...
                if prices:
                    for p in prices:
                        ...
                        conn.execute(
                            ...
                            "VALUES (?, 'D1', ?, ?, ?, ?, ?, ?)",
                            [sym, ts, p.get("open", 0), p.get("high", 0),
                             p.get("low", 0), p.get("close", 0), p.get("volume", 0)],
                        )
                    warmed += 1
    ...
    if tiered and any(tiered.values()):
        _ok(f"Tiered load: T1={len(tiered.get(1, []))} T2={len(tiered.get(2, []))} T3={len(tiered.get(3, []))}")
    else:
    ...
    ok1 = lock.transition("validation_test", "draft", "forward_testing", "backtest passed")
    ok2 = lock.transition("validation_test", "forward_testing", "live", "21d paper trading good")
    ...
    conn.execute("DELETE FROM strategies WHERE strategy_id IN ('validation_test', 'cas_test')")
