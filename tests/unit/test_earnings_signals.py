...
class TestEarningsSurpriseSignals:
    ...
    def test_eps_surprise_correct_sign(self):
        ...
        result = EarningsSurpriseSignals().compute(df)
        expected = (
            df.sort_values("report_date")["actual_eps"].values
            - df.sort_values("report_date")["estimated_eps"].values
        )
...
from quantstack.core.features.earnings_signals import EarningsImpliedMove
