...
async def train_ml_model(
    ...
    try:
        ...
        result = await asyncio.to_thread(
            _train_sync, symbol, model_type, feature_tiers,
            lookback_days, label_method, apply_causal_filter, save,
        )
        return result
...
def _train_sync(
    ...
    if ohlcv is None or len(ohlcv) < 100:
        return {"success": False, "error": f"Insufficient OHLCV data for {symbol} ({len(ohlcv) if ohlcv is not None else 0} bars)", "symbol": symbol}
    ...
    if label_col not in df.columns:
        return {"success": False, "error": f"Label column '{label_col}' not generated", "symbol": symbol}

    # Drop rows with NaN labels
    df = df.dropna(subset=[label_col])
    if len(df) < 50:
        return {"success": False, "error": f"Only {len(df)} labeled samples (need 50+)", "symbol": symbol}

    # Separate features and labels
    exclude_cols = {"open", "high", "low", "close", "volume", "signal",
                    "label_long", "label_short", "label_long_bars_to_exit",
                    "label_long_exit_type", "label_long_pnl_pct",
                    "label_short_bars_to_exit", "label_short_exit_type",
                    "label_short_pnl_pct"}
    ...
    if apply_causal_filter and len(feature_cols) > 15:
        try:
            ...
            cf_result = cf.filter(X, y)
            features_dropped = cf_result.dropped_features
            X = X[cf_result.surviving_features]
            logger.info(f"[ml] CausalFilter: {len(features_dropped)} features dropped for {symbol}")
    ...
    if save:
        ...
        metadata = {
            ...
            "accuracy": train_result.test_accuracy,
            "auc": train_result.test_auc,
            "cv_scores": {k: [round(v, 4) for v in vals] for k, vals in train_result.cv_scores.items()},
        ...
        logger.info(f"[ml] Model saved: {model_path} ({len(X.columns)} features, acc={train_result.test_accuracy:.3f})")
    ...
    return {
        ...
        "features_dropped": features_dropped,
        "cv_scores": {k: [round(v, 4) for v in vals] for k, vals in train_result.cv_scores.items()},
        "test_accuracy": round(train_result.test_accuracy, 4),
        "test_auc": round(train_result.test_auc, 4),
        "training_samples": len(X),
...
def _generate_labels(df: pd.DataFrame, method: str) -> pd.DataFrame:
    ...
    try:
        if method == "wave":
            from quantcore.labeling.wave_event_labeler import WaveEventLabeler
...
async def get_ml_model_status(
    ...
    try:
        if not _MODELS_DIR.exists():
            return {"success": True, "models": [], "total": 0, "message": "No models directory found"}
        ...
        for meta_path in meta_files:
            if not meta_path.exists():
                if symbol:
                    return {"success": True, "models": [], "total": 0, "message": f"No model found for {symbol}"}
                continue
            ...
            try:
                ...
                models.append({
                    "symbol": metadata.get("symbol", meta_path.stem.replace("_latest", "")),
...
def _predict_sync(symbol: str) -> dict[str, Any]:
    ...
    if not model_path.exists():
        return {"success": False, "error": f"No trained model for {symbol}", "symbol": symbol}
...
async def tune_hyperparameters(
    ...
    try:
        import asyncio
        result = await asyncio.to_thread(
            _tune_sync, symbol, model_type, n_trials, metric,
            feature_tiers, timeout_seconds,
        )
        return result
    except Exception as e:
...
def _tune_sync(
    ...
    ohlcv = store.load_ohlcv(symbol, Timeframe.D1)
    if ohlcv is None or len(ohlcv) < 100:
        return {"success": False, "error": f"Insufficient OHLCV for {symbol}", "symbol": symbol}
    ...
    df = df.dropna(subset=[label_col])
    if len(df) < 50:
        return {"success": False, "error": f"Only {len(df)} samples (need 50+)", "symbol": symbol}
...
async def register_model(
    ...
    from quant_pod.mcp._state import live_db_or_error
    ...
    try:
        ...
        ctx.db.execute(
            ...
            """,
            [
                registry_id, symbol, next_version,
                meta.get("model_type", "lightgbm"),
                meta.get("accuracy"), new_auc,
...
async def get_model_history(
    ...
    try:
        ...
        columns = [
            "registry_id", "model_version", "model_type", "status",
            "accuracy", "auc", "feature_names", "hyperparams", "data_window",
            "model_path", "trained_at", "promoted_at", "retired_at",
        ]
        versions = []
        ...
        return {"success": True, "symbol": symbol, "versions": versions, "total": len(versions)}
    except Exception as e:
...
async def rollback_model(
    ...
    try:
        ...
        if not target:
            return {"success": False, "error": f"Version {version} not found for {symbol}"}
        ...
        if target[1]:
            ...
            if src.exists():
                ...
                logger.warning(f"[ml] Model file not found at {src}, DB updated but file not restored")
...
async def compare_models(
    ...
    try:
        ...
        def _fetch_version(ver: int) -> dict[str, Any] | None:
            ...
            if not row:
                return None
            cols = ["model_version", "model_type", "status", "accuracy", "auc",
                    "feature_names", "hyperparams", "trained_at"]
        ...
        if not a:
            return {"success": False, "error": f"Version {version_a} not found for {symbol}"}
        if not b:
            return {"success": False, "error": f"Version {version_b} not found for {symbol}"}
        ...
        return {
            ...
            "version_a": a,
            "version_b": b,
            "accuracy_diff": round((b.get("accuracy") or 0) - (a.get("accuracy") or 0), 4),
...
def _check_drift_sync(symbol: str, window_days: int) -> dict[str, Any]:
    ...
    meta_path = _MODELS_DIR / f"{symbol}_latest.json"
    if not meta_path.exists():
        return {"success": False, "error": f"No model metadata for {symbol}", "symbol": symbol}
    ...
    if not feature_names:
        return {"success": False, "error": "No feature names in model metadata", "symbol": symbol}

    # Load full data window: training + recent
    store = DataStore()
    ohlcv = store.load_ohlcv(symbol, Timeframe.D1)
    if ohlcv is None or len(ohlcv) < window_days + 100:
        return {"success": False, "error": f"Insufficient data for drift check on {symbol}", "symbol": symbol}
    ...
    if not available_features:
        return {"success": False, "error": "No overlapping features found", "symbol": symbol}

    recent = df[available_features].tail(window_days).dropna()
    # Training window: everything before the recent window
    training = df[available_features].iloc[-(window_days + training_samples):-window_days].dropna()

    if len(recent) < 10 or len(training) < 30:
        return {"success": False, "error": "Insufficient data for KS test", "symbol": symbol}
    ...
    for feat in available_features:
        try:
            ...
            ks_stats[feat] = {"statistic": round(float(stat), 4), "p_value": round(float(p_val), 6)}
...
def _update_incremental_sync(symbol: str, new_data_days: int) -> dict[str, Any]:
    ...
    if not model_path.exists() or not meta_path.exists():
        return {"success": False, "error": f"No existing model for {symbol}", "symbol": symbol}
    ...
    if len(X_test) < 10:
        return {"success": False, "error": "Insufficient test samples", "symbol": symbol}
    ...
    if model_type == "lightgbm":
        ...
        return {"success": False, "error": f"Unsupported model_type: {model_type}", "symbol": symbol}
    ...
    if improved:
        # Save new model
        new_model_path = _MODELS_DIR / f"{symbol}_v{datetime.now(timezone.utc).strftime('%Y%m%d%H%M')}.joblib"
        _MODELS_DIR.mkdir(parents=True, exist_ok=True)
        ...
        try:
            ...
            if ctx and not db_err:
                ...
                ctx.db.execute(
                    ...
                    """,
                    [
                        reg_id, symbol, next_ver, model_type,
                        new_accuracy, new_auc,
                        json.dumps(feature_names),
                        json.dumps(new_metadata.get("hyperparams", {})),
                        json.dumps({"samples_added": samples_added, "incremental": True}),
                        str(new_model_path),
                    ],
                )
                registered_version = next_ver
                logger.info(f"[ml] Incremental model registered as v{next_ver} for {symbol}")
        except Exception as reg_exc:
            logger.warning(f"[ml] Registry update failed (model still saved): {reg_exc}")
...
def _review_model_quality_sync(
    ...
    resolved_model_path = Path(model_path) if model_path else _MODELS_DIR / f"{symbol}_latest.joblib"
    meta_path = resolved_model_path.with_suffix(".json")

    if not resolved_model_path.exists():
        return {"success": False, "error": f"Model not found: {resolved_model_path}", "symbol": symbol}
    if not meta_path.exists():
        return {"success": False, "error": f"Metadata not found: {meta_path}", "symbol": symbol}
    ...
    checks.append({
        ...
        "feedback": None if auc_passed else (
        ...
        if ohlcv is not None and len(ohlcv) >= 100:
            ...
            if label_col in df.columns:
                ...
                for col in [f for f in feature_names if f not in available]:
                    X_holdout[col] = 0.0
                X_holdout = X_holdout[feature_names].fillna(0).replace([np.inf, -np.inf], 0)
                ...
                if len(X_cal) >= 20:
                    ...
                    prob_true, prob_pred = calibration_curve(y_cal, proba, n_bins=5, strategy="uniform")
    ...
    checks.append({
        ...
        "threshold": 0.15,
        "feedback": None if calibration_passed else (
    ...
    try:
        if 'proba' in dir():
            pred_classes = (proba > 0.5).astype(int)  # noqa: F821 — only defined if calibration succeeded
            unique_preds = np.unique(pred_classes)
            if len(unique_preds) < 2:
                class_balance_passed = False
                min_class_ratio = 0.0
            else:
                min_class_ratio = float(min(np.mean(pred_classes), 1 - np.mean(pred_classes)))
    except Exception:
        pass
    checks.append({
        "name": "class_balance",
        ...
        "threshold": 0.10,
        "feedback": None if class_balance_passed else (
    ...
    checks.append({
        ...
        "threshold": 0.15,
        "feedback": None if temporal_passed else (
    ...
    critical_failures = [c for c in checks if not c["passed"] and c["name"] in ("auc_gate", "class_balance")]
    fixable_failures = [c for c in checks if not c["passed"] and c["name"] not in ("auc_gate", "class_balance")]
    ...
    feedback_str = " ".join(failure_feedbacks) if failure_feedbacks else "Model passes all quality checks."
...
async def train_stacking_ensemble(
    ...
    try:
        ...
        return await asyncio.to_thread(
            _train_stacking_sync, symbol, base_models, meta_learner, feature_tiers,
        )
    except Exception as e:
...
def _train_stacking_sync(
    ...
    for mt in models_list:
        if mt == "lightgbm":
            ...
            import catboost as cb
            estimators.append(("catboost", cb.CatBoostClassifier(
                verbose=0, random_state=42,
            )))
        else:
    ...
    if len(estimators) < 2:
        return {"success": False, "error": "Need at least 2 valid base models for stacking", "symbol": symbol}
    ...
    for i, n1 in enumerate(pred_names):
        ...
        for n2 in pred_names:
            corr_val = float(np.corrcoef(base_predictions[n1], base_predictions[n2])[0, 1])
    ...
    if hasattr(stack.final_estimator_, "coef_"):
        ...
        meta_weights = [round(float(w), 4) for w in stack.final_estimator_.feature_importances_]
    ...
    return {
        ...
        "warning": "Ensemble underperforms best single model" if ensemble_benefit < 0 else None,
    }
...
async def compute_and_store_features(
    ...
    try:
        import asyncio
        return await asyncio.to_thread(
            _compute_and_store_features_sync, symbol, tiers, lookback_days,
        )
    except Exception as e:
...
def _compute_and_store_features_sync(
    ...
    ctx, db_err = live_db_or_error()
    if db_err:
        return {"success": False, "error": f"DB unavailable: {db_err}", "symbol": symbol}
    ...
    for idx, row in df_features.iterrows():
        ...
        features_json = json.dumps({col: round(float(val), 6) for col, val in row.items()})
        ctx.db.execute(
            """
...
async def get_feature_lineage(
    ...
    try:
        ...
        if model_version is not None:
            ...
            if not row:
                return {"success": False, "error": f"No registry entry for {symbol} v{model_version}", "symbol": symbol}

            feature_names_raw = row[0]
            feature_names = json.loads(feature_names_raw) if isinstance(feature_names_raw, str) else feature_names_raw or []
            data_window = json.loads(row[2]) if isinstance(row[2], str) else row[2] or {}
        ...
        if db_err:
            ...
            return {"success": False, "error": "No feature data found", "symbol": symbol}
        ...
        return {
            "success": True,
            "symbol": symbol,
            "feature_names": list(features_json.keys()) if isinstance(features_json, dict) else [],
            "feature_version": feature_version,
...
async def train_cross_sectional_model(
    ...
    try:
        import asyncio
        return await asyncio.to_thread(
            _train_cross_sectional_sync, symbols, target, feature_tiers, lookback_days,
        )
    except Exception as e:
...
def _train_cross_sectional_sync(
    ...
    if fwd_days is None:
        return {"success": False, "error": f"Unknown target '{target}'. Use: {list(target_map.keys())}"}
    ...
    for sym in symbols:
        try:
            ...
            if ohlcv is None or len(ohlcv) < 100:
                logger.warning(f"[ml] Skipping {sym}: insufficient data ({len(ohlcv) if ohlcv is not None else 0} bars)")
                continue
    ...
    if len(symbols_loaded) < 3:
        return {"success": False, "error": f"Only {len(symbols_loaded)} symbols loaded (need 3+)"}

    # --- Stack into panel ---
    panel = pd.concat(panel_frames, axis=0)
    panel = panel.dropna(subset=[target])

    exclude_cols = {"open", "high", "low", "close", "volume", "signal", target, "_symbol"}
    feature_cols = [c for c in panel.columns if c not in exclude_cols and panel[c].dtype in ("float64", "float32", "int64", "int32")]

    if len(feature_cols) < 3:
        return {"success": False, "error": f"Only {len(feature_cols)} features — need more"}
    ...
    if len(X) < 100:
        return {"success": False, "error": f"Only {len(X)} valid panel rows (need 100+)"}
...
async def train_deep_model(
    ...
    try:
        ...
        result = await asyncio.to_thread(
            _train_deep_sync, symbol, architecture, target, sequence_length, epochs,
        )
        return result
    except Exception as e:
...
def _train_deep_sync(
    ...
    from quantcore.data.storage import DataStore
    from quantcore.features.technical_indicators import TechnicalIndicators
    from quantcore.models.tft_predictor import HORIZONS, TFTReturnPredictor
    ...
    if not feature_cols:
        ...
        feature_cols = ["returns", "volatility", "momentum_5", "momentum_20", "volume_change"]
