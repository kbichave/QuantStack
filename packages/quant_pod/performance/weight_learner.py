...
_DEFAULT_PROFILES: dict[str, dict[str, float]] = {
    "trending_up": {"trend": 0.35, "rsi": 0.10, "macd": 0.20, "bb": 0.05, "sentiment": 0.10, "ml": 0.15, "flow": 0.05},
    "trending_down": {"trend": 0.30, "rsi": 0.15, "macd": 0.20, "bb": 0.05, "sentiment": 0.10, "ml": 0.15, "flow": 0.05},
    "ranging": {"trend": 0.05, "rsi": 0.25, "macd": 0.10, "bb": 0.25, "sentiment": 0.10, "ml": 0.15, "flow": 0.10},
    "unknown": {"trend": 0.15, "rsi": 0.15, "macd": 0.15, "bb": 0.15, "sentiment": 0.10, "ml": 0.20, "flow": 0.10},
}
...
class WeightLearner:
    ...
    def learn_weights(
        ...
        if not trades:
            logger.info("[WeightLearner] No trades with signal snapshots — keeping defaults")
        ...
        for regime, regime_trades in by_regime.items():
            if len(regime_trades) < self._min_trades:
                ...
                learned_profiles[regime] = _DEFAULT_PROFILES.get(regime, _DEFAULT_PROFILES["unknown"])
    ...
    def _fit_regime(
        ...
        if len(set(y)) < 2:
            logger.debug(f"[WeightLearner] {regime}: all trades same outcome — using defaults")
            return _DEFAULT_PROFILES.get(regime, _DEFAULT_PROFILES["unknown"]), {
                "source": "default", "reason": "single_class", "n_trades": len(trades)
            }
        ...
        if not non_zero_cols.any():
            return _DEFAULT_PROFILES.get(regime, _DEFAULT_PROFILES["unknown"]), {
                "source": "default", "reason": "zero_variance", "n_trades": len(trades)
            }
        ...
        try:
            ...
            coefs = np.abs(model.coef_[0])  # abs because we want importance, not direction
            coef_sum = coefs.sum()

            if coef_sum <= 0:
                return _DEFAULT_PROFILES.get(regime, _DEFAULT_PROFILES["unknown"]), {
                    "source": "default", "reason": "zero_coefs", "n_trades": len(trades)
                }
            ...
            if split >= 5 and len(X_scaled) - split >= 5:
                model_val = LogisticRegression(C=1.0, penalty="l2", solver="lbfgs", max_iter=200, random_state=42)
                ...
                if oos_accuracy < 0.5:
                    ...
                    return _DEFAULT_PROFILES.get(regime, _DEFAULT_PROFILES["unknown"]), {
                        "source": "default", "reason": f"oos_accuracy={oos_accuracy:.3f}",
                        "n_trades": len(trades),
                    }
            else:
                oos_accuracy = None
            ...
            return weights, {
                ...
                "oos_accuracy": round(oos_accuracy, 3) if oos_accuracy is not None else None,
            ...
            return _DEFAULT_PROFILES.get(regime, _DEFAULT_PROFILES["unknown"]), {
                "source": "default", "reason": "sklearn_missing", "n_trades": len(trades)
            }
        except Exception as exc:
            logger.warning(f"[WeightLearner] {regime}: fit failed ({exc}) — using defaults")
            return _DEFAULT_PROFILES.get(regime, _DEFAULT_PROFILES["unknown"]), {
                "source": "default", "reason": str(exc), "n_trades": len(trades)
            }
    ...
    def _reconstruct_votes(
        self, technical: dict, regime: dict, sentiment: dict,
    ) -> dict[str, float]:
        ...
        votes["vote_trend"] = {"trending_up": 1.0, "trending_down": -1.0}.get(trend, 0.0)
        ...
        votes["vote_macd"] = (1.0 if macd and macd > 0 else -1.0) if macd is not None else 0.0
    ...
    def _ensure_table(self) -> None:
        ...
        try:
            ...
            """)
            self._conn.execute("CREATE SEQUENCE IF NOT EXISTS synthesis_weights_seq START 1")
        except Exception:
            pass
    ...
    def _persist_weights(
        ...
        try:
            self._conn.execute(
                ...
                """,
                [effective_date, json.dumps(profiles), json.dumps(metadata, default=str)],
            )
