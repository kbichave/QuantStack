...
def pre_trade_forecast(
    ...
    if adv <= 0 or arrival_price <= 0:
        logger.warning(f"[TCA] Invalid inputs for {symbol}: adv={adv}, price={arrival_price}")
        adv = max(adv, 1)
    ...
    commission_bps = (commission_per_share * shares / notional) * 10000 if notional > 0 else 0
...
class TCAEngine:
    ...
    def record_arrival(
        ...
        logger.debug(f"[TCA] Arrival recorded: {symbol} {side.value} {shares}sh @ {arrival_price}")
    ...
    def aggregate_report(self) -> dict:
        ...
        report: dict = {
            ...
            "std_shortfall_vs_arrival_bps": float(np.std(arrivals)) if len(arrivals) > 1 else 0.0,
