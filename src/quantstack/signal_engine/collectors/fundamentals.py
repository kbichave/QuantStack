...
def _collect_fundamentals_sync(symbol: str, store: DataStore) -> dict[str, Any]:
    ...
    try:
        row = (
            store.get_fundamentals(symbol)
            if hasattr(store, "get_fundamentals")
            else None
        )
    except Exception as exc:
    ...
    try:
        ...
        if "gross_profit" in df.columns and "total_assets" in df.columns:
            try:
                ...
                if (
                    ...
                    and "revenue" in gp_input.columns
                ):
                    gp_input["cost_of_revenue"] = (
                        gp_input["revenue"] - gp_input["gross_profit"]
                    )
        ...
        if "total_assets" in df.columns:
            try:
                ...
                logger.debug(
                    f"[fundamentals] {symbol}: AssetGrowthAnomaly failed: {exc}"
                )
        ...
        if "revenue" in df.columns and len(df) >= 4:
            try:
                ...
                logger.debug(
                    f"[fundamentals] {symbol}: RevenueAcceleration failed: {exc}"
                )
        ...
        if "revenue" in df.columns and len(df) >= 4:
            ebitda_col = (
                ...
                else ("operating_income" if "operating_income" in df.columns else None)
            )
            if ebitda_col:
                try:
                    ol_input = df.copy()
                    ...
                    logger.debug(
                        f"[fundamentals] {symbol}: OperatingLeverage failed: {exc}"
                    )
        ...
        if all(
            c in df.columns for c in ("operating_cash_flow", "capital_expenditures")
        ):
            try:
        ...
        if all(c in df.columns for c in _piotroski_cols) and len(df) >= 2:
            try:
                ...
                if (
                    ...
                    and "gross_profit" in pio_input.columns
                    and "revenue" in pio_input.columns
                ):
                    pio_input["cost_of_revenue"] = (
                        pio_input["revenue"] - pio_input["gross_profit"]
                    )
                if "cost_of_revenue" in pio_input.columns:
                    ...
                    result["piotroski_short_signal"] = int(
                        pio_df["short_signal"].iloc[-1]
                    )
            except Exception as exc:
        ...
        if all(c in df.columns for c in _beneish_cols) and len(df) >= 5:
            try:
                ...
                result["beneish_manipulation"] = int(
                    ben_df["manipulation_risk"].iloc[-1]
                )
            except Exception as exc:
...
def _add_analyst_signals(symbol: str, store: DataStore, result: dict) -> None:
    ...
    try:
        ...
        if (
            "estimate_date" not in rename_map.values()
            or "eps_estimate" not in rename_map.values()
        ):
            return
...
def _add_herding_signals(symbol: str, store: DataStore, result: dict) -> None:
    ...
    try:
        ...
        agg: dict[str, Any] = (
            {"total_holders": ("investor_name", "nunique")}
            if "investor_name" in raw_df.columns
            else {"total_holders": (raw_df.columns[0], "count")}
        )

        period_df = (
            raw_df.groupby(date_col)
            .agg(
                total_holders=pd.NamedAgg(
                    column=(
                        raw_df.columns[1]
                        if "investor_name" not in raw_df.columns
                        else "investor_name"
                    ),
                    aggfunc="nunique",
                ),
        ...
        if change_col:
            inc = (
                raw_df.groupby(date_col)
                .apply(lambda g: (g[change_col] > 0).sum())
                .reset_index(name="holders_increased")
            )
            dec = (
                raw_df.groupby(date_col)
                .apply(lambda g: (g[change_col] < 0).sum())
                .reset_index(name="holders_decreased")
            )
...
def _add_earnings_surprise_signals(symbol: str, store: DataStore, result: dict) -> None:
    ...
    try:
        ...
        logger.debug(
            f"[fundamentals] {symbol}: earnings surprise signals failed: {exc}"
        )
...
def _add_composite_signals(symbol: str, store: DataStore, result: dict) -> None:
    ...
    try:
        ...
        if not qm_df.empty:
            ...
            result["quality_momentum_long"] = (
                int(qm_long) if pd.notna(qm_long) else None
            )
            qm_short = last.get("quality_momentum_short")
            result["quality_momentum_short"] = (
                int(qm_short) if pd.notna(qm_short) else None
            )
    except Exception as exc:
    ...
    try:
        ...
        if not em_df.empty:
            ...
            result["earnings_momentum_long"] = (
                int(em_long) if pd.notna(em_long) else None
            )
            em_short = last.get("earnings_momentum_short")
            result["earnings_momentum_short"] = (
                int(em_short) if pd.notna(em_short) else None
            )
    except Exception as exc:
        logger.debug(
            f"[fundamentals] {symbol}: EarningsMomentumComposite failed: {exc}"
        )
