...
class StrategyValidationFlow(Flow[ValidationState]):
    ...
    def load_historical_data(self) -> dict[str, Any]:
        ...
        logger.info(f"[Validation] Loaded {n_bars} daily bars for {len(symbols)} symbols")
        ...
        self.state.returns_df = pd.DataFrame(
            {"portfolio": portfolio_returns, **{s: returns_dict[s] for s in returns_dict}}
        )
    ...
    def run_overfitting_checks(self, wf_result: dict) -> dict[str, Any]:
        ...
        try:
            from quantcore.research.overfitting import probability_of_backtest_overfitting
    ...
    def route_on_fitness(self, checks_result: dict) -> ValidationVerdict:
        ...
        if dsr_not_genuine and pbo_overfit:
            ...
            self.state.detail = (
                f"DSR={dsr.dsr:.3f} (not genuine) and PBO={pbo.pbo:.3f} > {_PBO_THRESHOLD}"
            )
            return "overfit"
        ...
        if overfit_ratio > _OVERFIT_RATIO_THRESHOLD:
            ...
            self.state.detail = (
                f"Overfit ratio={overfit_ratio:.2f} > threshold {_OVERFIT_RATIO_THRESHOLD}"
            )
            return "degraded"
