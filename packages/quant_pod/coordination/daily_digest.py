...
class DailyDigest:
    ...
    def format_discord(self, report: DigestReport) -> dict[str, Any]:
        ...
        fields = [
            ...
            {"name": "Realized P&L", "value": f"${report.total_realized_pnl:,.2f}", "inline": True},
            {"name": "Strategies", "value": f"L:{report.total_live} FT:{report.total_forward_testing} D:{report.total_draft}", "inline": True},
            {"name": "Loop Iterations", "value": f"F:{report.factory_iterations} T:{report.trader_iterations} ML:{report.ml_iterations}", "inline": True},
            {"name": "Errors", "value": str(report.total_loop_errors), "inline": True},
        ]

        if report.strategies_promoted:
            fields.append({"name": "Promoted", "value": ", ".join(report.strategies_promoted), "inline": False})
        if report.breakers_tripped:
            fields.append({"name": "Breakers Tripped", "value": ", ".join(report.breakers_tripped), "inline": False})
        ...
        return {
            "embeds": [{
                ...
                "color": color,
                "fields": fields,
                "footer": {"text": f"Generated at {report.generated_at.strftime('%H:%M UTC')}"},
            }]
        }
    ...
    def _fill_trades(self, report: DigestReport, start: datetime, end: datetime) -> None:
        try:
    ...
    def _fill_strategies(self, report: DigestReport, start: datetime, end: datetime) -> None:
        try:
    ...
    def _fill_loop_health(self, report: DigestReport, start: datetime, end: datetime) -> None:
        try:
    ...
    def _fill_events(self, report: DigestReport, start: datetime, end: datetime) -> None:
        try:
