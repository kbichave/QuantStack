...
class Hypothesis(BaseModel):
    ...
    hypothesis_id: str = ""
    thesis: str = Field(
        ..., description="Clear, testable statement about a market relationship"
    )
    source: Literal[
        "factor_decay",
        "regime_gap",
    ...
    target_regimes: list[str] = Field(
        default_factory=lambda: ["trending_up", "ranging"]
    )
...
class FailureAnalysis(BaseModel):
    ...
    next_action: Literal[
        "modify_and_retry", "abandon", "pivot_hypothesis", "need_more_data"
    ]
