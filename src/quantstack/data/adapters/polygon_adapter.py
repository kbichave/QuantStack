...
class PolygonAdapter(AssetClassAdapter):
    ...
    def fetch_options_chain(
        ...
        api_key = (
            ...
            if hasattr(self._provider, "api_key")
            else os.getenv("POLYGON_API_KEY", "")
        )
        if not api_key:
            return None
        ...
        try:
            ...
            for snap in client.list_snapshot_options_chain(
                ...
                try:
                    ...
                    bid = (
                        float(last_quote.bid) if last_quote and last_quote.bid else None
                    )
                    ask = (
                        float(last_quote.ask) if last_quote and last_quote.ask else None
                    )
                    ...
                    contracts.append(
                        {
                            ...
                            "delta": (
                                float(greeks.delta) if greeks and greeks.delta else None
                            ),
                            "gamma": (
                                float(greeks.gamma) if greeks and greeks.gamma else None
                            ),
                            "theta": (
                                float(greeks.theta) if greeks and greeks.theta else None
                            ),
                            "vega": (
                                float(greeks.vega) if greeks and greeks.vega else None
                            ),
                            "open_interest": (
                                int(snap.open_interest) if snap.open_interest else None
                            ),
            ...
            logger.info(
                f"[PolygonAdapter] fetched {len(contracts)} option contracts for {symbol}"
            )
    ...
    def _fetch(
        ...
        start: date | None = startdate.date() if start_date else None
        end: date | None = enddate.date() if end_date else None
