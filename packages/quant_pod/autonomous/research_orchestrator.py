...
class ResearchOrchestrator:
    ...
    def __init__(
        ...
        self._conn = conn
        self._watchlist = watchlist or ["SPY", "QQQ", "AAPL", "MSFT", "NVDA", "XOM", "JPM", "UNH", "AMZN", "GOOGL"]
    ...
    async def run_nightly(self) -> OrchestratorReport:
        ...
        try:
            ...
            if health.overall_status == "CRITICAL":
                logger.critical("[Orchestrator] CRITICAL health — aborting nightly research")
    ...
    async def run_weekly(self) -> OrchestratorReport:
        ...
        # Step 1: ML Scientist
        try:
            from quant_pod.research.ml_scientist import MLScientist
            scientist = MLScientist(self._conn)
            ml_plan = await scientist.generate_plan()
            report.experiments_designed = len(ml_plan.experiments)
            report.steps_completed.append(f"ml_scientist_{len(ml_plan.experiments)}_experiments")
        except Exception as exc:
    ...
    async def run_monthly(self) -> OrchestratorReport:
        ...
        try:
            from quant_pod.research.execution_researcher import ExecutionResearcher
    ...
    async def _execute_ml_experiments(self, plan: Any) -> int:
        ...
        for exp in plan.experiments:
            try:
                ...
                logger.warning(f"[Orchestrator] Experiment {exp.experiment_id} failed: {exc}")
    ...
    def _log_experiment(self, exp: Any, result: dict) -> None:
        ...
        try:
            exp_id = getattr(exp, "experiment_id", "") or f"mlexp_{_uuid.uuid4().hex[:8]}"
            self._conn.execute(
                """
                ...
                [
                    ...
                    json.dumps((getattr(exp, "config", {}) or {}).get("feature_tiers", [])),
                    result.get("test_auc"),
