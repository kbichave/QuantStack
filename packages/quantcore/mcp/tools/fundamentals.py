...
async def get_sec_filing_items(
    ...
    try:
        ...
        fp.close()
        return {"error": f"Failed to fetch {filing_type} filings for {ticker}: {e}", "ticker": ticker}

    if filings_df.empty:
        fp.close()
        return {"error": f"No {filing_type} filings found for {ticker}", "ticker": ticker}

    accession_number = filings_df.iloc[0].get("accession_number")
    if not accession_number:
        fp.close()
        return {"error": f"No accession number in {filing_type} filing for {ticker}", "ticker": ticker}
