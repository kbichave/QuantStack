...
from quant_pod.mcp.tools.backtesting import _fetch_price_data, _generate_signals_from_rules
...
async def get_strategy_gaps() -> dict[str, Any]:
    ...
    try:
        ...
        regime_strategies: dict[str, list[dict[str, Any]]] = {r: [] for r in _KNOWN_REGIMES}
        ...
        for regime, strategies in regime_strategies.items():
            ...
            known_sharpes = [
                s for s in (trailing_sharpe.get(s["strategy_id"]) for s in strategies)
                if s is not None
            ]
            if known_sharpes:
        ...
        return {
            ...
            "trailing_sharpe": {k: round(v, 3) for k, v in trailing_sharpe.items() if v is not None},
        }
    except Exception as e:
...
async def promote_draft_strategies(
    ...
    try:
        ...
        for row in drafts:
            ...
            overfit_ratio = (is_sharpe / oos_sharpe) if oos_sharpe > 1e-9 else float("inf")
...
async def check_strategy_rules(
    ...
    try:
        ...
        if signals_df.empty:
            return {"success": False, "error": "Signal generation produced empty result"}
        ...
        exit_triggered = any(r["passed"] for r in exit_detail if r.get("type") not in ("time_stop", "take_profit", "stop_loss"))
