...
def _fetch_ibkr_book(symbol: str, n_levels: int = 5) -> dict | None:
    ...
    try:
        ...
        client_id = int(os.environ.get("IBKR_L2_CLIENT_ID", "2"))  # separate client from main
...
def collect_l2_microstructure(
    ...
    if close is not None and volume is not None and len(close) >= kyle_period:
        try:
            ...
            last = kl_df.iloc[-1]
            result["l2_kyle_lambda"] = float(last["kyle_lambda"]) if not np.isnan(last["kyle_lambda"]) else None
            result["l2_kyle_zscore"] = float(last["kyle_lambda_zscore"]) if not np.isnan(last["kyle_lambda_zscore"]) else None
