...
"""
Shared tool implementations — importable by langchain tools, functions layer,
and autonomous modules without creating circular dependencies.

Leaf in the dependency graph: imports only from quantstack.db,
quantstack.tools._state, quantstack.tools._helpers, quantstack.core.*.
"""
...
async def get_strategy_impl(
    ...
    try:
        ...
        logger.error(f"[_shared] get_strategy failed: {e}")
...
async def register_strategy_impl(
    ...
    try:
        ...
        logger.info(f"[_shared] Registered strategy {strategy_id}: {name}")
        return {"success": True, "strategy_id": strategy_id, "status": "draft"}
    except Exception as e:
        logger.error(f"[_shared] register_strategy failed: {e}")
        return {"success": False, "error": str(e)}
...
async def run_backtest_impl(
    ...
    try:
        ...
        logger.error(f"[_shared] run_backtest failed: {e}")
        return {
            "success": False,
            "error": str(e),
