...
class DarkPoolSignals:
    ...
    def compute(self, dark_pool_data: pd.DataFrame) -> pd.DataFrame:
        ...
        if "close" in dark_pool_data.columns:
            price_premium = (dark_pool_data["dark_pool_price"] - dark_pool_data["close"].shift(1)) / dark_pool_data["close"].shift(1)
        else:
...
class BorrowRateSignals:
    ...
    def compute(self, borrow_data: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        required = {"borrow_rate_pct", "short_interest", "shares_float", "days_to_cover"}
        ...
        borrow_rising = (rate.rolling(5).mean() > rate.rolling(5).mean().shift(5)).astype(int)
        ...
        return pd.DataFrame(
            {
                ...
                "squeeze_risk":    (borrow_data["days_to_cover"] > self.squeeze_threshold).astype(int),
...
_LM_POSITIVE = {
    "strong", "growth", "increase", "improved", "favorable", "record",
    "profitable", "expanding", "accelerating", "outperform", "exceed",
    "robust", "solid", "momentum", "gain", "success", "effective",
    "confident", "outstanding", "exceptional", "excellent",
}

_LM_NEGATIVE = {
    "decline", "decrease", "loss", "risk", "uncertain", "challenge",
    "adverse", "impair", "delay", "weak", "difficult", "pressure",
    "headwind", "shortage", "disruption", "volatile", "concern",
    "disappointing", "difficult", "cautious", "challenging",
}

_UNCERTAINTY = {
    "may", "might", "could", "possibly", "uncertain", "unclear",
    "expect", "anticipate", "believe", "appears", "seems", "approximately",
}
