...
class AutonomousRunner:
    ...
    async def run(
        ...
        if sys_status.get("kill_switch_active"):
            logger.warning(f"[AutonomousRunner] run={run_id} ABORTED — kill switch active")
            return self._empty_report(run_id, started_at, symbols or [])

        if sys_status.get("risk_halted"):
            logger.warning(f"[AutonomousRunner] run={run_id} ABORTED — daily risk limit breached")
            return self._empty_report(run_id, started_at, symbols or [])
        ...
        if not symbols:
            logger.info(f"[AutonomousRunner] run={run_id} — empty symbol list, nothing to do")
        ...
        results: list[SymbolResult] = await asyncio.gather(*tasks, return_exceptions=False)
    ...
    def _check_drift(self, strategy: dict, brief: dict):
        ...
        try:
            ...
            logger.debug(f"[AutonomousRunner] drift check failed for {strategy_id}: {exc}")
            return None
    ...
    async def _handle_rule_based(
        ...
        if action == "skip":
            self._audit_skip(symbol, f"rule-based: bias neutral/weak ({brief.market_bias})", brief, run_id)
            return SymbolResult(
                symbol=symbol, path=DecisionPath.RULE_BASED,
                path_reason=reason, action=None,
                reasoning=reasoning, elapsed_seconds=time.monotonic() - t0,
            )
    ...
    async def _submit_order(
        ...
        paper_mode = self._paper_only or (
            os.getenv("USE_REAL_TRADING", "false").strip().lower() not in ("true", "1", "yes")
        )
        ...
        try:
            from quant_pod.mcp.tools.execution import execute_trade
    ...
    def _load_active_strategies(self) -> list[dict]:
        ...
        try:
            ...
            cols = [
                "strategy_id", "name", "description", "status",
                "regime_affinity", "parameters", "entry_rules",
                "exit_rules", "risk_params", "asset_class",
            ]
            strategies = []
            for row in rows:
                d = dict(zip(cols, row))
                # Deserialize JSON fields
                for key in ("regime_affinity", "parameters", "entry_rules", "exit_rules", "risk_params"):
    ...
    def _load_portfolio_snapshot(self) -> dict:
        ...
        try:
            from quant_pod.mcp._state import require_ctx
            ctx = require_ctx()
            ...
            positions = {
                p.symbol: {
                    ...
                    "unrealized_pnl_pct": (
                        ...
                        if p.entry_price and p.entry_price > 0 else 0.0
                    ),
                }
    ...
    async def _get_system_status(self) -> dict:
        ...
        try:
            from quant_pod.mcp.tools.analysis import get_system_status
    ...
    @staticmethod
    def _empty_report(run_id: str, started_at: datetime, symbols: list[str]) -> RunReport:
...
def _parse_args() -> Any:
    ...
    parser = argparse.ArgumentParser(description="AutonomousRunner — headless trading loop")
    parser.add_argument("--symbols", nargs="+", default=None, help="Symbols to analyze")
    parser.add_argument("--dry-run", action="store_true", help="Log decisions, submit no orders")
    parser.add_argument("--paper-only", action="store_true", default=True, help="Force paper mode")
