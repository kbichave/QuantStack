...
class EquityTracker:
    ...
    def snapshot_daily(self, as_of: date | None = None) -> dict[str, Any]:
        ...
        if existing:
            logger.info(f"[EquityTracker] Snapshot for {snapshot_date} already exists — skipping")
        ...
        # Daily return %
        daily_return_pct = (daily_pnl / prev_equity * 100) if prev_equity and prev_equity > 0 else 0.0

        # High water mark and drawdown
        hwm = self._get_high_water_mark()
        new_hwm = max(hwm, total_equity)
        drawdown_pct = ((total_equity - new_hwm) / new_hwm * 100) if new_hwm > 0 else 0.0
    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        total_return_pct = ((last_equity - first_equity) / first_equity * 100) if first_equity > 0 else 0
        avg_daily = sum(returns) / n_days if n_days > 0 else 0
        std_daily = (sum((r - avg_daily) ** 2 for r in returns) / max(n_days - 1, 1)) ** 0.5
    ...
    def _get_cash(self) -> float:
        row = self._conn.execute("SELECT cash FROM cash_balance WHERE id = 1").fetchone()
