...
class TCAStore:
    ...
    def get_aggregate_stats(
        ...
        (
            trade_count, avg_slip, median_slip, worst_slip, best_slip,
            total_dollar, fav_pct, avg_vwap,
        ) = result_stats
        ...
        return {
            ...
            "median_slippage_bps": round(median_slip, 2) if median_slip is not None else None,
            "worst_slippage_bps": round(worst_slip, 2) if worst_slip is not None else None,
            "best_slippage_bps": round(best_slip, 2) if best_slip is not None else None,
            "total_shortfall_dollar": round(total_dollar, 2) if total_dollar is not None else 0.0,
