...
class AgentHardener:
    ...
    def measure_ic_agreement(self, ic_outputs: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        bias_label = (
            ...
            if avg_bias > 0.3
                ...
                "bearish"
                if avg_bias < -0.3
                else "conflicted" if len(set(biases)) > 1 else "neutral"
            )
        )
    ...
    def validate_agent_output(
        ...
        if action and action not in valid_actions:
            violations.append(
                f"Unknown action '{action}' — expected one of {valid_actions}"
            )
    ...
    def reconcile_blackboard_with_portfolio(
        ...
        for sym in bb_symbols - port_symbols:
            phantom.append(
                {"symbol": sym, "bb_quantity": bb_map[sym].get("quantity", 0)}
            )

        for sym in port_symbols - bb_symbols:
            unknown.append(
                {"symbol": sym, "portfolio_quantity": port_map[sym].get("quantity", 0)}
            )
