...
from quant_pod.mcp._state import live_db_or_error, _serialize
...
async def execute_options_trade(
    ...
    try:
        ...
        try:
            ...
            return {"success": False, "error": f"Invalid expiry_date: {expiry_date}. Use YYYY-MM-DD."}

        dte = (exp - date.today()).days
        if dte < 0:
            return {"success": False, "error": f"Expiry {expiry_date} is in the past (DTE={dte})."}
        ...
        premium_per_share = _bs_price(underlying_price, strike, T, r, sigma, option_type)
        ...
        premium_at_risk = total_premium if action == "buy" else total_premium * 5  # Conservative for shorts
        equity = snapshot.total_equity

        violations = []

        # Premium at risk check
        max_premium_pct = float(os.getenv("RISK_MAX_PREMIUM_AT_RISK_PCT", "0.02").split("#")[0].strip())
        ...
        daily_loss_pct = float(os.getenv("RISK_DAILY_LOSS_LIMIT_PCT", "0.02").split("#")[0].strip())
        ...
        if paper_mode or get_broker_mode() == "paper":
            ...
            fill_result = _execute_alpaca_options(
                symbol, option_type, strike, expiry_date, action, contracts,
        ...
        if strategy_id:
            try:
                ctx.conn.execute(
                    ...
                    """,
                    [
                        strategy_id, symbol, f"{action}_{option_type}",
                        fill_result.get("fill_premium_per_share", premium_per_share),
                        datetime.now(), ctx.session_id,
                    ],
                )
        ...
        return {
            ...
            "total_premium": round(total_premium, 2),
            "max_risk": round(total_premium if action == "buy" else total_premium * 5, 2),
            **fill_result,
...
def _execute_alpaca_options(
    symbol: str, option_type: str, strike: float, expiry_date: str,
    action: str, contracts: int, order_type: str, limit_price: float | None,
) -> dict[str, Any]:
