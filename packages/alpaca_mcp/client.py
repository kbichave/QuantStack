...
def _require_alpaca() -> None:
    if not _ALPACA_AVAILABLE:
        raise ImportError(
            "alpaca-py is required for AlpacaBrokerClient. "
...
class AlpacaBrokerClient(BrokerInterface):
    ...
    def get_balance(self, account_id: str) -> UnifiedBalance:
        ...
        return UnifiedBalance(
            ...
            day_trade_buying_power = float(acct.daytrading_buying_power) if hasattr(acct, "daytrading_buying_power") else None,
            maintenance_margin = float(acct.maintenance_margin) if hasattr(acct, "maintenance_margin") else None,
        )
