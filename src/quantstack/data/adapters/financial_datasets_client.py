    ...
    def acquire(self) -> None:
        with self._lock:
            ...
            if len(self._timestamps) >= self._max:
                ...
                if sleep_for > 0:
                    logger.debug(
                        f"[FinancialDatasets] Rate limit reached, sleeping {sleep_for:.1f}s"
                    )
        ...
        for attempt in range(_MAX_RETRIES):
            ...
            try:
                ...
                if resp.status_code == 429:
                    retry_after = float(
                        resp.headers.get("Retry-After", _RETRY_BACKOFF_BASE)
                    )
                ...
                last_exc = exc
                logger.warning(
                    f"[FinancialDatasets] HTTP {exc.response.status_code}: {url}"
                )
