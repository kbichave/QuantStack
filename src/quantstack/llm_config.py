...
def _classify_tier(agent_name: str) -> str:
    ...
    if agent_name.endswith("_ic"):
        return "ic"
    logger.debug(
        f"[llm_config] Unknown tier for '{agent_name}', defaulting to 'assistant'"
    )
    return "assistant"
...
def _provider_available(provider: str) -> bool:
    ...
    if check is None:
        logger.warning(
            f"[llm_config] Unknown provider '{provider}' — treating as unavailable"
        )
        return False
    return check()
...
def _resolve_provider(tier: str) -> str:
    ...
    for provider in chain:
        ...
        model = builder(tier)
        logger.debug(
            f"[llm_config] Resolved provider='{provider}' tier='{tier}' → {model}"
        )
        return model
...
def get_llm_for_role(role: str) -> Any:
    ...
    if role not in _TIER_ENV_OVERRIDE:
        raise ValueError(
            f"Unknown role '{role}'. Valid roles: {list(_TIER_ENV_OVERRIDE)}"
        )
