...
class AlpacaBrokerClient(BrokerInterface):
    ...
    def get_balance(self, account_id: str) -> UnifiedBalance:
        ...
        return UnifiedBalance(
            ...
            day_trade_buying_power=(
                float(acct.daytrading_buying_power)
                if hasattr(acct, "daytrading_buying_power")
                else None
            ),
            maintenance_margin=(
                float(acct.maintenance_margin)
    ...
    def place_order(self, account_id: str, order: UnifiedOrder) -> UnifiedOrderResult:
            ...
            if order.order_type == "market":
                ...
                if order.limit_price is None:
                    raise BrokerOrderError(
                        "limit_price required for limit orders", order
                    )
                ...
                raise BrokerOrderError(
                    f"Unsupported order type: {order.order_type}", order
                )
        ...
        return UnifiedOrderResult(
            ...
            client_order_id=(
                str(result.client_order_id) if result.client_order_id else None
            ),
