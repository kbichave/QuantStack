...
def _load_returns(symbols: list[str], lookback_days: int) -> pd.DataFrame:
    ...
    end_dt = datetime.now()
    start_dt = end_dt - timedelta(days=int(lookback_days * 1.5))  # overshoot for weekends/holidays
    ...
    if len(returns) < 20:
        raise ValueError(f"Only {len(returns)} overlapping return observations — need at least 20")
...
def _portfolio_stats(
    ...
    rc = w * marginal
    rc_total = rc.sum()
    risk_contributions = {s: round(float(rc[i] / rc_total) * 100, 2) for i, s in enumerate(syms)}
...
async def optimize_portfolio(
    ...
    if method not in _VALID_METHODS:
        return {"success": False, "error": f"Unknown method '{method}'. Choose from: {sorted(_VALID_METHODS)}"}
    if len(symbols) < 2:
        return {"success": False, "error": "Need at least 2 symbols for portfolio optimization"}
...
async def compute_hrp_weights(
    ...
    def _compute() -> dict[str, Any]:
        ...
        for row in link:
            left_idx, right_idx, dist, count = int(row[0]), int(row[1]), float(row[2]), int(row[3])
            left_label = syms[left_idx] if left_idx < len(syms) else f"cluster_{left_idx}"
            right_label = syms[right_idx] if right_idx < len(syms) else f"cluster_{right_idx}"
        ...
        pairs.sort()
        lowest_corr = [{"pair": f"{s1}/{s2}", "correlation": round(c, 4)} for c, s1, s2 in pairs[:3]]
        highest_corr = [{"pair": f"{s1}/{s2}", "correlation": round(c, 4)} for c, s1, s2 in pairs[-3:]]
