...
class BenchmarkTracker:
    ...
    def update_benchmark(
        ...
        if close_price is None:
            ...
            return {"error": "no_price", "benchmark": benchmark, "date": str(snapshot_date)}
        ...
        cumulative = ((close_price - first_close) / first_close * 100) if first_close > 0 else 0.0
        ...
        self._conn.execute(
            ...
            VALUES (?, ?, ?, ?, ?)
            """,
            [snapshot_date, benchmark, round(close_price, 4), round(daily_return, 4), round(cumulative, 4)],
        )

        logger.debug(f"[Benchmark] {benchmark} {snapshot_date}: ${close_price:.2f} ({daily_return:+.2f}%)")
        return {
    ...
    def compute_comparison(
        ...
        for window in windows:
            ...
            self._conn.execute(
                ...
                """,
                [
                    snapshot_date, benchmark, window,
                    comp["portfolio_sharpe"], comp["benchmark_sharpe"],
                    comp["portfolio_sortino"], comp["alpha"], comp["beta"],
                ],
            )
            results.append(comp)
        ...
        if results:
            logger.info(
                ...
                + " | ".join(f"{r['window']}d Sharpe={r['portfolio_sharpe']:.2f} α={r['alpha']:.2f}" for r in results)
            )
        return results
    ...
    def get_comparison(
        ...
        columns = [
            "date", "benchmark", "window_days", "portfolio_sharpe",
            "benchmark_sharpe", "portfolio_sortino", "alpha", "beta",
        ]
    ...
    def _compute_window(
        self, benchmark: str, window: int, as_of: date,
    ) -> dict[str, Any] | None:
        ...
        port_sortino = (port_mean / port_down_std * math.sqrt(252)) if port_down_std > 0 else 0

        # Beta = Cov(port, bench) / Var(bench)
        cov = sum((p - port_mean) * (b - bench_mean) for p, b in zip(port_rets, bench_rets)) / max(n - 1, 1)
